...
static void USBD_CMPSIT_AssignEp(USBD_HandleTypeDef *pdev, uint8_t Add,
...
static void USBD_CMPSIT_MSCDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
                                __IO uint32_t *Sze, uint8_t speed);
#endif /* USBD_CMPSIT_ACTIVATE_MSC == 1U */

#if USBD_CMPSIT_ACTIVATE_CDC == 1U
static void USBD_CMPSIT_CDCDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
                                __IO uint32_t *Sze, uint8_t speed);
#endif /* USBD_CMPSIT_ACTIVATE_CDC == 1U */

#if USBD_CMPSIT_ACTIVATE_DFU == 1U
static void USBD_CMPSIT_DFUDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
                                __IO uint32_t *Sze, uint8_t speed);
#endif /* USBD_CMPSIT_ACTIVATE_DFU == 1U */

#if USBD_CMPSIT_ACTIVATE_RNDIS == 1U
static void USBD_CMPSIT_RNDISDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
                                  __IO uint32_t *Sze, uint8_t speed);
#endif /* USBD_CMPSIT_ACTIVATE_RNDIS == 1U */

#if USBD_CMPSIT_ACTIVATE_CDC_ECM == 1U
static void USBD_CMPSIT_CDC_ECMDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
                                    __IO uint32_t *Sze, uint8_t speed);
#endif /* USBD_CMPSIT_ACTIVATE_CDC_ECM == 1U */

#if USBD_CMPSIT_ACTIVATE_AUDIO == 1U
static void USBD_CMPSIT_AUDIODesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
                                  __IO uint32_t *Sze, uint8_t speed);
#endif /* USBD_CMPSIT_ACTIVATE_AUDIO == 1U */

#if USBD_CMPSIT_ACTIVATE_CUSTOMHID == 1
static void USBD_CMPSIT_CUSTOMHIDDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
                                      __IO uint32_t *Sze, uint8_t speed);
#endif /* USBD_CMPSIT_ACTIVATE_CUSTOMHID == 1U */

#if USBD_CMPSIT_ACTIVATE_VIDEO == 1U
static void USBD_CMPSIT_VIDEODesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
                                  __IO uint32_t *Sze, uint8_t speed);
#endif /* USBD_CMPSIT_ACTIVATE_VIDEO == 1U */

#if USBD_CMPSIT_ACTIVATE_PRINTER == 1U
static void USBD_CMPSIT_PRNTDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
                                 __IO uint32_t *Sze, uint8_t speed);
#endif /* USBD_CMPSIT_ACTIVATE_PRINTER == 1U */

#if USBD_CMPSIT_ACTIVATE_CCID == 1U
static void USBD_CMPSIT_CCIDDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
                                 __IO uint32_t *Sze, uint8_t speed);
#endif /* USBD_CMPSIT_ACTIVATE_CCID == 1U */

#if USBD_CMPSIT_ACTIVATE_MTP == 1U
static void USBD_CMPSIT_MTPDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
                                __IO uint32_t *Sze, uint8_t speed);
 ...
 * @{
 */
/* This structure is used only for the Configuration descriptors and Device
 * Qualifier */
...
__ALIGN_BEGIN static uint8_t
    USBD_CMPSIT_FSCfgDesc[USBD_CMPST_MAX_CONFDESC_SZ] __ALIGN_END = {0};
static uint8_t *pCmpstFSConfDesc = USBD_CMPSIT_FSCfgDesc;
/* Variable that dynamically holds the current size of the configuration
 * descriptor */
static __IO uint32_t CurrFSConfDescSz = 0U;

#ifdef USE_USB_HS
__ALIGN_BEGIN static uint8_t
    USBD_CMPSIT_HSCfgDesc[USBD_CMPST_MAX_CONFDESC_SZ] __ALIGN_END = {0};
...
__ALIGN_BEGIN static uint8_t
    USBD_CMPSIT_DeviceQualifierDesc[USB_LEN_DEV_QUALIFIER_DESC] __ALIGN_END = {
 ...
 * @param  class: type of the class to be added (from
...
uint8_t USBD_CMPSIT_AddClass(USBD_HandleTypeDef *pdev,
                             ...
                             USBD_CompositeClassTypeDef class, uint8_t cfgidx) {
    if ((pdev->classId < USBD_MAX_SUPPORTED_CLASS) &&
        ...
        /* Call configuration descriptor builder and endpoint configuration
         * builder */
...
uint8_t USBD_CMPSIT_AddToConfDesc(USBD_HandleTypeDef *pdev) {
    ...
    /* For the first class instance, start building the config descriptor common
     * part */
    if (pdev->classId == 0U) {
    ...
    switch (pdev->tclasslist[pdev->classId].ClassType) {
        ...
        case CLASS_TYPE_HID:
            /* Setup Max packet sizes (for HID, no dependency on USB Speed, both
             * HS/FS have same packet size) */
            pdev->tclasslist[pdev->classId].CurrPcktSze = HID_EPIN_SIZE;

            /* Find the first available interface slot and Assign number of
             * interfaces */
            ...
            /* Assign IN Endpoint */
            USBD_CMPSIT_AssignEp(pdev, iEp, USBD_EP_TYPE_INTR,
                                 pdev->tclasslist[pdev->classId].CurrPcktSze);

            /* Configure and Append the Descriptor */
            USBD_CMPSIT_HIDMouseDesc(pdev, (uint32_t)pCmpstFSConfDesc,
                                     &CurrFSConfDescSz,
                                     (uint8_t)USBD_SPEED_FULL);

#ifdef USE_USB_HS
            USBD_CMPSIT_HIDMouseDesc(pdev, (uint32_t)pCmpstHSConfDesc,
                                     &CurrHSConfDescSz,
        ...
        case CLASS_TYPE_MSC:
            ...
            /* Set IN endpoint slot */
            iEp = pdev->tclasslist[pdev->classId].EpAdd[0];
            USBD_CMPSIT_AssignEp(pdev, iEp, USBD_EP_TYPE_BULK,
                                 pdev->tclasslist[pdev->classId].CurrPcktSze);
            ...
            /* Configure and Append the Descriptor */
            USBD_CMPSIT_MSCDesc(pdev, (uint32_t)pCmpstFSConfDesc,
                                &CurrFSConfDescSz, (uint8_t)USBD_SPEED_FULL);

#ifdef USE_USB_HS
            USBD_CMPSIT_MSCDesc(pdev, (uint32_t)pCmpstHSConfDesc,
                                &CurrHSConfDescSz, (uint8_t)USBD_SPEED_HIGH);
#endif /* USE_USB_HS */
        ...
        case CLASS_TYPE_CDC:
            ...
            /* Configure and Append the Descriptor */
            USBD_CMPSIT_CDCDesc(pdev, (uint32_t)pCmpstFSConfDesc,
                                &CurrFSConfDescSz, (uint8_t)USBD_SPEED_FULL);

#ifdef USE_USB_HS
            USBD_CMPSIT_CDCDesc(pdev, (uint32_t)pCmpstHSConfDesc,
                                &CurrHSConfDescSz, (uint8_t)USBD_SPEED_HIGH);
#endif /* USE_USB_HS */
        ...
        case CLASS_TYPE_DFU:
            /* Setup Max packet sizes (for DFU, no dependency on USB Speed, both
             * HS/FS have same packet size) */
            ...
            /* Configure and Append the Descriptor */
            USBD_CMPSIT_DFUDesc(pdev, (uint32_t)pCmpstFSConfDesc,
                                &CurrFSConfDescSz, (uint8_t)USBD_SPEED_FULL);

#ifdef USE_USB_HS
            USBD_CMPSIT_DFUDesc(pdev, (uint32_t)pCmpstHSConfDesc,
                                &CurrHSConfDescSz, (uint8_t)USBD_SPEED_HIGH);
#endif /* USE_USB_HS */
        ...
        case CLASS_TYPE_RNDIS:
            ...
            /* Configure and Append the Descriptor */
            USBD_CMPSIT_RNDISDesc(pdev, (uint32_t)pCmpstFSConfDesc,
                                  &CurrFSConfDescSz, (uint8_t)USBD_SPEED_FULL);

#ifdef USE_USB_HS
            USBD_CMPSIT_RNDISDesc(pdev, (uint32_t)pCmpstHSConfDesc,
                                  &CurrHSConfDescSz, (uint8_t)USBD_SPEED_HIGH);
#endif /* USE_USB_HS */
        ...
        case CLASS_TYPE_ECM:
            ...
            /* Configure and Append the Descriptor */
            USBD_CMPSIT_CDC_ECMDesc(pdev, (uint32_t)pCmpstFSConfDesc,
                                    &CurrFSConfDescSz,
                                    (uint8_t)USBD_SPEED_FULL);

#ifdef USE_USB_HS
            USBD_CMPSIT_CDC_ECMDesc(pdev, (uint32_t)pCmpstHSConfDesc,
                                    &CurrHSConfDescSz,
                                    (uint8_t)USBD_SPEED_HIGH);
#endif /* USE_USB_HS */
        ...
        case CLASS_TYPE_AUDIO:
            ...
            pdev->tclasslist[pdev->classId].CurrPcktSze =
                USBD_AUDIO_GetEpPcktSze(pdev, 0U, 0U);
            ...
            /* Assign OUT Endpoint */
            USBD_CMPSIT_AssignEp(pdev, iEp, USBD_EP_TYPE_ISOC,
                                 pdev->tclasslist[pdev->classId].CurrPcktSze);

            /* Configure and Append the Descriptor (only FS mode supported) */
            USBD_CMPSIT_AUDIODesc(pdev, (uint32_t)pCmpstFSConfDesc,
                                  &CurrFSConfDescSz, (uint8_t)USBD_SPEED_FULL);
        ...
        case CLASS_TYPE_CHID:
            ...
            /* Configure and Append the Descriptor */
            USBD_CMPSIT_CUSTOMHIDDesc(pdev, (uint32_t)pCmpstFSConfDesc,
                                      &CurrFSConfDescSz,
                                      (uint8_t)USBD_SPEED_FULL);

#ifdef USE_USB_HS
            USBD_CMPSIT_CUSTOMHIDDesc(pdev, (uint32_t)pCmpstHSConfDesc,
                                      &CurrHSConfDescSz,
                                      (uint8_t)USBD_SPEED_HIGH);
#endif /* USE_USB_HS */
        ...
        case CLASS_TYPE_VIDEO:
            ...
            /* Configure and Append the Descriptor */
            USBD_CMPSIT_VIDEODesc(pdev, (uint32_t)pCmpstFSConfDesc,
                                  &CurrFSConfDescSz, (uint8_t)USBD_SPEED_FULL);

#ifdef USE_USB_HS
            USBD_CMPSIT_VIDEODesc(pdev, (uint32_t)pCmpstHSConfDesc,
                                  &CurrHSConfDescSz, (uint8_t)USBD_SPEED_HIGH);
#endif /* USE_USB_HS */
        ...
        case CLASS_TYPE_PRINTER:
            ...
            /* Configure and Append the Descriptor */
            USBD_CMPSIT_PRNTDesc(pdev, (uint32_t)pCmpstFSConfDesc,
                                 &CurrFSConfDescSz, (uint8_t)USBD_SPEED_FULL);

#ifdef USE_USB_HS
            USBD_CMPSIT_PRNTDesc(pdev, (uint32_t)pCmpstHSConfDesc,
                                 &CurrHSConfDescSz, (uint8_t)USBD_SPEED_HIGH);
#endif /* USE_USB_HS */
        ...
        case CLASS_TYPE_CCID:
            ...
            iEp = pdev->tclasslist[pdev->classId].EpAdd[2];
            USBD_CMPSIT_AssignEp(pdev, iEp, USBD_EP_TYPE_INTR,
                                 CCID_CMD_PACKET_SIZE);

            /* Configure and Append the Descriptor */
            USBD_CMPSIT_CCIDDesc(pdev, (uint32_t)pCmpstFSConfDesc,
                                 &CurrFSConfDescSz, (uint8_t)USBD_SPEED_FULL);

#ifdef USE_USB_HS
            USBD_CMPSIT_CCIDDesc(pdev, (uint32_t)pCmpstHSConfDesc,
                                 &CurrHSConfDescSz, (uint8_t)USBD_SPEED_HIGH);
#endif /* USE_USB_HS */
        ...
        case CLASS_TYPE_MTP:
            ...
            /* Configure and Append the Descriptor */
            USBD_CMPSIT_MTPDesc(pdev, (uint32_t)pCmpstFSConfDesc,
                                &CurrFSConfDescSz, (uint8_t)USBD_SPEED_FULL);

#ifdef USE_USB_HS
            USBD_CMPSIT_MTPDesc(pdev, (uint32_t)pCmpstHSConfDesc,
                                &CurrHSConfDescSz, (uint8_t)USBD_SPEED_HIGH);
#endif /* USE_USB_HS */
...
static void USBD_CMPSIT_AddConfDesc(uint32_t Conf, __IO uint32_t *pSze) {
    ...
    ptr->bmAttributes =
        0xC0U; /* bmAttributes: Self Powered according to user configuration */
#else
    ptr->bmAttributes =
...
static void USBD_CMPSIT_HIDMouseDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
    ...
    *Sze += (uint32_t)sizeof(USBD_HIDDescTypeDef);
    ...
    __USBD_CMPSIT_SET_EP(pdev->tclasslist[pdev->classId].Eps[0].add,
                         USBD_EP_TYPE_INTR, HID_EPIN_SIZE, HID_HS_BINTERVAL,
                         HID_FS_BINTERVAL);
...
static void USBD_CMPSIT_MSCDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
    ...
    /* Append Endpoint descriptor to Configuration descriptor */
    __USBD_CMPSIT_SET_EP(
        (pdev->tclasslist[pdev->classId].Eps[0].add), (USBD_EP_TYPE_BULK),
        (pdev->tclasslist[pdev->classId].CurrPcktSze), (0U), (0U));

    /* Append Endpoint descriptor to Configuration descriptor */
    __USBD_CMPSIT_SET_EP(
        (pdev->tclasslist[pdev->classId].Eps[1].add), (USBD_EP_TYPE_BULK),
        (pdev->tclasslist[pdev->classId].CurrPcktSze), (0U), (0U));
...
static void USBD_CMPSIT_CDCDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
    ...
    *Sze += (uint32_t)sizeof(USBD_IadDescTypeDef);
    ...
    __USBD_CMPSIT_SET_IF(pdev->tclasslist[pdev->classId].Ifs[0], 0U, 1U, 0x02,
                         0x02U, 0x01U, 0U);
    ...
    *Sze += (uint32_t)sizeof(USBD_CDCUnionFuncDescTypeDef);
    ...
    __USBD_CMPSIT_SET_IF(pdev->tclasslist[pdev->classId].Ifs[1], 0U, 2U, 0x0A,
                         0U, 0U, 0U);
...
static void USBD_CMPSIT_DFUDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
    ...
    for (idx = 0U; idx < USBD_DFU_MAX_ITF_NUM; idx++) {
        ...
        __USBD_CMPSIT_SET_IF(
            pdev->tclasslist[pdev->classId].Ifs[0], (uint8_t)idx, 0U, 0xFEU,
            0x01U, 0x02U, (uint8_t)USBD_IDX_INTERFACE_STR + 1U + (uint8_t)idx);
    }
...
static void USBD_CMPSIT_CDC_ECMDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
    ...
    *Sze += (uint32_t)sizeof(USBD_IadDescTypeDef);
    ...
    __USBD_CMPSIT_SET_IF(pdev->tclasslist[pdev->classId].Ifs[0], 0U, 1U, 0x02U,
                         0x06U, 0U, 0U);
    ...
    *Sze += (uint32_t)sizeof(USBD_CDCUnionFuncDescTypeDef);
    ...
    /* Append ECM Communication IN Endpoint Descriptor to Configuration
     * descriptor */
    __USBD_CMPSIT_SET_EP(pdev->tclasslist[pdev->classId].Eps[2].add,
                         USBD_EP_TYPE_INTR, CDC_ECM_CMD_PACKET_SIZE,
                         CDC_ECM_HS_BINTERVAL, CDC_ECM_FS_BINTERVAL);

    /* Append ECM Data class interface descriptor to Configuration descriptor */
    __USBD_CMPSIT_SET_IF(pdev->tclasslist[pdev->classId].Ifs[1], 0U, 2U, 0x0AU,
                         0U, 0U, 0U);

    if (speed == (uint8_t)USBD_SPEED_HIGH) {
        pdev->tclasslist[pdev->classId].CurrPcktSze =
            CDC_ECM_DATA_HS_MAX_PACKET_SIZE;
    }
...
static void USBD_CMPSIT_AUDIODesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
    ...
    *Sze += (uint32_t)sizeof(USBD_IadDescTypeDef);
    ...
    /* Append AUDIO Interface descriptor to Configuration descriptor */
    __USBD_CMPSIT_SET_IF(pdev->tclasslist[pdev->classId].Ifs[0], 0U, 0U,
                         USB_DEVICE_CLASS_AUDIO, AUDIO_SUBCLASS_AUDIOCONTROL,
                         AUDIO_PROTOCOL_UNDEFINED, 0U);

    /* Append AUDIO USB Speaker Class-specific AC Interface descriptor to
    ...
    *Sze += (uint32_t)sizeof(USBD_IfDescTypeDef);
    ...
    /* Append USB Speaker Input Terminal Descriptor to Configuration
     * descriptor*/
    ...
    *Sze += (uint32_t)sizeof(USBD_SpeakerInDescTypeDef);
    ...
    /*Append USB Speaker Audio Feature Unit Descriptor to Configuration
     * descriptor */
    ...
    *Sze += (uint32_t)sizeof(USBD_SpeakerFeatureDescTypeDef);
    ...
    /*Append USB Speaker Output Terminal Descriptor to Configuration
     * descriptor*/
    ...
    *Sze += (uint32_t)sizeof(USBD_SpeakerOutDescTypeDef);
    ...
    __USBD_CMPSIT_SET_IF(pdev->tclasslist[pdev->classId].Ifs[1], 0U, 0U,
    ...
    __USBD_CMPSIT_SET_IF(pdev->tclasslist[pdev->classId].Ifs[1], 0x01U, 0x01U,
                         USB_DEVICE_CLASS_AUDIO, AUDIO_SUBCLASS_AUDIOSTREAMING,
                         AUDIO_PROTOCOL_UNDEFINED, 0U);
...
static void USBD_CMPSIT_RNDISDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
    ...
    *Sze += (uint32_t)sizeof(USBD_CDCUnionFuncDescTypeDef);
    ...
    /* Append Endpoint descriptor to Configuration descriptor */
    __USBD_CMPSIT_SET_EP(pdev->tclasslist[pdev->classId].Eps[2].add,
                         USBD_EP_TYPE_INTR, CDC_RNDIS_CMD_PACKET_SIZE,
                         CDC_RNDIS_HS_BINTERVAL, CDC_RNDIS_FS_BINTERVAL);
    ...
    if (speed == (uint8_t)USBD_SPEED_HIGH) {
        pdev->tclasslist[pdev->classId].CurrPcktSze =
            CDC_RNDIS_DATA_HS_MAX_PACKET_SIZE;
    }
...
static void USBD_CMPSIT_CUSTOMHIDDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
    ...
    /* Control Interface Descriptor */
    __USBD_CMPSIT_SET_IF(pdev->tclasslist[pdev->classId].Ifs[0], 0U, 2U, 3U, 0U,
                         0U, 0U);
    ...
    *Sze += (uint32_t)sizeof(USBD_DescTypeDef);
    ...
    /* Append Endpoint descriptor to Configuration descriptor */
    __USBD_CMPSIT_SET_EP(pdev->tclasslist[pdev->classId].Eps[0].add,
                         USBD_EP_TYPE_INTR, CUSTOM_HID_EPIN_SIZE,
                         CUSTOM_HID_HS_BINTERVAL, CUSTOM_HID_FS_BINTERVAL);
...
static void USBD_CMPSIT_VIDEODesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
    ...
    __ALIGN_BEGIN static uint8_t usbd_uvc_guid[16] __ALIGN_END = {
        DBVAL(UVC_UNCOMPRESSED_GUID),
        0x00,
        0x00,
        0x10,
        0x00,
    ...
    *Sze += (uint32_t)sizeof(USBD_IadDescTypeDef);
    ...
    __USBD_CMPSIT_SET_IF(pdev->tclasslist[pdev->classId].Ifs[0], 0U, 0U,
                         UVC_CC_VIDEO, 1U, PC_PROTOCOL_UNDEFINED, 0U);

    /* Append Class-specific VC Interface Descriptor to Configuration
     * descriptor*/
    ...
    *Sze += (uint32_t)sizeof(USBD_OutputTerminalDescTypeDef);
    ...
    __USBD_CMPSIT_SET_IF(pdev->tclasslist[pdev->classId].Ifs[1], 0U, 0U,
                         UVC_CC_VIDEO, SC_VIDEOSTREAMING, PC_PROTOCOL_UNDEFINED,
                         0U);

    /* Append Class-specific VS Header Descriptor (Input) to Configuration
     * descriptor */
    pSpHeaDesc = ((USBD_ClassSpecificVsHeaderDescTypeDef *)(pConf + *Sze));
    pSpHeaDesc->bLength =
        (uint8_t)sizeof(USBD_ClassSpecificVsHeaderDescTypeDef);
    ...
    *Sze += (uint32_t)sizeof(USBD_ColorMatchingDescTypeDef);
    ...
    /* Interface 1, Alternate Setting 1*/
    __USBD_CMPSIT_SET_IF(1U, 1U, 1U, UVC_CC_VIDEO, SC_VIDEOSTREAMING,
                         PC_PROTOCOL_UNDEFINED, 0U);
...
static void USBD_CMPSIT_PRNTDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
    ...
    /* Control Interface Descriptor */
    __USBD_CMPSIT_SET_IF(pdev->tclasslist[pdev->classId].Ifs[0], 0U, 0x02, 0x07,
                         0x01U, USB_PRNT_BIDIRECTIONAL, 0U);

    if (speed == (uint8_t)USBD_SPEED_HIGH) {
        pdev->tclasslist[pdev->classId].CurrPcktSze =
            PRNT_DATA_HS_MAX_PACKET_SIZE;
    }
...
static void USBD_CMPSIT_CCIDDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
    ...
    /* Control Interface Descriptor */
    __USBD_CMPSIT_SET_IF(pdev->tclasslist[pdev->classId].Ifs[0], 0U, 0x03,
                         0x0BU, 0U, 0U, 0U);
    ...
    *Sze += (uint32_t)sizeof(USBD_CCID_DescTypeDef);
    ...
    __USBD_CMPSIT_SET_EP(pdev->tclasslist[pdev->classId].Eps[2].add,
                         ...
                         CCID_CMD_HS_BINTERVAL, CCID_CMD_FS_BINTERVAL);
...
static void USBD_CMPSIT_MTPDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
    ...
    __USBD_CMPSIT_SET_IF((pdev->tclasslist[pdev->classId].Ifs[0]), (0U),
                         (uint8_t)(pdev->tclasslist[pdev->classId].NumEps),
                         USB_MTP_INTRERFACE_CLASS, USB_MTP_INTRERFACE_SUB_CLASS,
                         USB_MTP_INTRERFACE_PROTOCOL, (0U));

    if (speed == (uint8_t)USBD_SPEED_HIGH) {
        pdev->tclasslist[pdev->classId].CurrPcktSze =
            MTP_DATA_MAX_HS_PACKET_SIZE;
    }
    ...
    /* Append Endpoint descriptor to Configuration descriptor */
    __USBD_CMPSIT_SET_EP(pdev->tclasslist[pdev->classId].Eps[2].add,
                         USBD_EP_TYPE_INTR, MTP_CMD_PACKET_SIZE,
                         MTP_HS_BINTERVAL, MTP_FS_BINTERVAL);
 ...
 *         Find and set the class ID relative to selected class type and
 * instance
 * @param  pdev: device instance
 * @param  Class: Class type, can be CLASS_TYPE_NONE if requested to find class
 * from setup request
 * @param  Instance: Instance number of the class (0 if first/unique instance,
 * >0 otherwise)
 * @retval The Class ID, The pdev->classId is set with the value of the selected
 * class ID.
 */
uint32_t USBD_CMPSIT_SetClassID(USBD_HandleTypeDef *pdev,
    ...
    for (idx = 0U; idx < pdev->NumClasses; idx++) {
         ...
         * active */
        if (((USBD_CompositeClassTypeDef)(pdev->tclasslist[idx].ClassType) ==
             Class) &&
...
uint8_t USBD_CMPST_ClearConfDesc(USBD_HandleTypeDef *pdev) {
    ...
    /* Reset the configuration descriptor pointer to default value and its size
     * to zero */
