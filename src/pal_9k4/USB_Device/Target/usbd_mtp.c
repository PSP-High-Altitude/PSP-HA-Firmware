 ...
 *           - Enumeration as MTP Device (and enumeration for each implemented
 *memory interface)
 ...
 *Media Transfer Protocol Revision 1.1 April 6, 2011" This driver implements the
 *following aspects of the specification:
 *             - Device descriptor management
 *             - Configuration descriptor management
 *             - Enumeration as MTP device with 2 data endpoints (IN and OUT)
...
static uint8_t USBD_MTP_Setup(USBD_HandleTypeDef *pdev,
...
__ALIGN_BEGIN static uint8_t USBD_MTP_CfgDesc[MTP_CONFIG_DESC_SIZ] __ALIGN_END =
    {
        ...
        LOBYTE(MTP_CONFIG_DESC_SIZ), /* wTotalLength: Total size of the Config
                                        descriptor */
        HIBYTE(MTP_CONFIG_DESC_SIZ), 0x01, /* bNumInterfaces: 1 interface */
        ...
        0x03,                     /* bNumEndpoints:  */
        USB_MTP_INTRERFACE_CLASS, /* bInterfaceClass: bInterfaceClass: user's
                                     interface for MTP */
        USB_MTP_INTRERFACE_SUB_CLASS, /* bInterfaceSubClass:Abstract Control
                                         Model */
        ...
        LOBYTE(MTP_CMD_PACKET_SIZE), HIBYTE(MTP_CMD_PACKET_SIZE),
        MTP_FS_BINTERVAL /* Polling interval in milliseconds */
};

/* USB Standard Device Descriptor */
__ALIGN_BEGIN static uint8_t
    USBD_MTP_DeviceQualifierDesc[USB_LEN_DEV_QUALIFIER_DESC] __ALIGN_END = {
...
static uint8_t USBD_MTP_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx) {
    ...
    hmtp =
        (USBD_MTP_HandleTypeDef *)USBD_malloc(sizeof(USBD_MTP_HandleTypeDef));
    ...
    /* Open EP OUT */
    (void)USBD_LL_OpenEP(pdev, MTPOutEpAdd, USBD_EP_TYPE_BULK,
                         hmtp->MaxPcktLen);
    pdev->ep_out[MTPOutEpAdd & 0xFU].is_used = 1U;

    /* Open INTR EP IN */
    (void)USBD_LL_OpenEP(pdev, MTPCmdEpAdd, USBD_EP_TYPE_INTR,
                         MTP_CMD_PACKET_SIZE);
        ...
        case USB_REQ_TYPE_CLASS:
            switch (req->bRequest) {
                case MTP_REQ_CANCEL:
                    ...
                    (void)USBD_CtlPrepareRx(pdev, (uint8_t *)(hmtp->rx_buff),
                                            len);
                    break;
                ...
                case MTP_REQ_RESET:
                    ...
                    (void)USBD_LL_PrepareReceive(pdev, MTPOutEpAdd,
                                                 (uint8_t *)&hmtp->rx_buff,
                                                 hmtp->MaxPcktLen);
                    break;
                ...
                case MTP_REQ_GET_DEVICE_STATUS:
                    switch (hmtp->MTP_ResponsePhase) {
                        case MTP_READ_DATA:
                            len = 4U;
                            hmtp->dev_status =
                                ((uint32_t)MTP_RESPONSE_DEVICE_BUSY << 16) |
                                len;
                            break;
                        ...
                        case MTP_RECEIVE_DATA:
                            len = 4U;
                            hmtp->dev_status =
                                ((uint32_t)MTP_RESPONSE_TRANSACTION_CANCELLED
                                 << 16) |
                                len;
                            break;
                        ...
                        case MTP_PHASE_IDLE:
                            len = 4U;
                            hmtp->dev_status =
                                ((uint32_t)MTP_RESPONSE_OK << 16) | len;
                            break;

                        default:
                            break;
                    }
                    (void)USBD_CtlSendData(pdev, (uint8_t *)&hmtp->dev_status,
                                           len);
                    break;
            ...
            switch (req->bRequest) {
                case USB_REQ_GET_INTERFACE:
                    ...
                    if (pdev->dev_state == USBD_STATE_CONFIGURED) {
                        ...
                        (void)USBD_CtlSendData(
                            pdev, (uint8_t *)&hmtp->alt_setting, 1U);
                    }
                    break;
                ...
                case USB_REQ_CLEAR_FEATURE:
                    ...
                    if ((((uint8_t)req->wIndex) & 0x80U) == 0x80U) {
                        (void)USBD_LL_OpenEP(pdev, ((uint8_t)req->wIndex),
                                             USBD_EP_TYPE_BULK,
                                             hmtp->MaxPcktLen);
                    } else {
                        (void)USBD_LL_OpenEP(pdev, ((uint8_t)req->wIndex),
    ...
    if (epnum == (MTPInEpAdd & 0x7FU)) {
        switch (hmtp->MTP_ResponsePhase) {
            ...
            case MTP_READ_DATA:
                ...
                (void)USBD_LL_PrepareReceive(pdev, MTPInEpAdd,
                                             (uint8_t *)&hmtp->rx_buff, len);
                break;
...
static uint8_t *USBD_MTP_GetHSCfgDesc(uint16_t *length) {
    ...
    USBD_EpDescTypeDef *pEpOutDesc =
        USBD_GetEpDesc(USBD_MTP_CfgDesc, MTP_OUT_EP);
    USBD_EpDescTypeDef *pEpCmdDesc =
        USBD_GetEpDesc(USBD_MTP_CfgDesc, MTP_CMD_EP);
...
uint8_t USBD_MTP_RegisterInterface(USBD_HandleTypeDef *pdev,
    ...
    pdev->classId = USBD_CMPSIT_GetClassID(pdev, CLASS_TYPE_MTP, 0U);
