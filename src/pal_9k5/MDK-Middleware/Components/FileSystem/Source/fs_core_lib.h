...
#define fsDevCtrlCodeBlockCount \
    255 /* Retrieve total number of blocks on the volume */
#define fsDevCtrlCodeErasedValue \
    254 /* Retrieve the value of erased memory content   */
#define fsDevCtrlCodeProgramUnit \
    253 /* Retrieve smallest programmable unit in bytes  */
...
extern uint32_t ram_ReadSector(uint32_t sect, uint8_t *buf, uint32_t cnt,
                               RAM_DEV *ram);
extern uint32_t ram_WriteSector(uint32_t sect, const uint8_t *buf, uint32_t cnt,
                                RAM_DEV *ram);
...
extern uint32_t usbh_msc_ReadSector(uint32_t sect, uint32_t cnt, uint8_t *buf,
                                    uint32_t instance);
extern uint32_t usbh_msc_WriteSector(uint32_t sect, uint32_t cnt,
                                     const uint8_t *buf, uint32_t instance);
extern uint32_t usbh_msc_ReadInfo(fsMediaInfo *info, uint32_t instance);
extern fsStatus usbh_msc_DevCtrl(fsDevCtrlCode code, void *p,
                                 uint32_t instance);
...
extern uint32_t fsj_write(FSJOUR *fsj, uint32_t sect, uint32_t rsec,
                          uint8_t *buf);
...
extern fsStatus efs_rename(const char *filename, const char *newname,
                           fsEFS_Volume *vol);
...
extern fsStatus fat_delete(const char *fn, const char *options,
                           fsFAT_Volume *vol);
extern fsStatus fat_ffind(const char *fn, fsFileInfo *info, fsFAT_Volume *vol);
extern fsStatus fat_rename(const char *path, const char *newname,
                           fsFAT_Volume *vol);
extern fsStatus fat_attrib(fsFAT_Volume *vol, const char *path,
                           uint32_t attrib);
...
extern fsStatus fat_rmdir(const char *path, const char *options,
                          fsFAT_Volume *vol);
...
extern fsStatus fat_time_set(const char *path, fsTime *create, fsTime *access,
                             fsTime *write, fsFAT_Volume *vol);
extern fsStatus fat_time_get(const char *path, fsTime *create, fsTime *access,
                             fsTime *write, fsFAT_Volume *vol);
