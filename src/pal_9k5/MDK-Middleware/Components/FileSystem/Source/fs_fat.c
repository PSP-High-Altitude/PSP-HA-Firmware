...
static const uint8_t ChIndex[13] = {1,  3,  5,  7,  9,  14, 16,
                                    18, 20, 22, 24, 28, 30};
#endif
...
/**
  ...
  \return     true if found opened file with specified flags set, false
  otherwise
*/
...
/**
  ...
  \param[in]  time                      pointer to FS_TIME structure containing
  current time \return     Packed time value
*/
...
/**
  ...
  \param[in]  time                      Pointer to FS_TIME structure containing
  current date \return     Packed time value
*/
...
/**
  ...
  Note that only members day, month and year are populated in the fsTime
  structure.

  \param[in]  date                      date information in FAT entry format
  \param[out] td                        fsTime structure where date will be
  stored
*/
...
/**
  ...
  Note that only members hour, minute and second are populated in the fsTime
  structure.

  \param[in]  time                      time information in FAT entry format
  \param[out] td                        fsTime structure where time will be
  stored
*/
...
/**
  ...
  \param[out] dst                       pointer to FileName field in the SFN
  entry \param[in]  src                       name to embed into SFN entry
*/
...
/**
  ...
  \param[in]  name                      character array containing file or
  directory name \param[in]  len                       name length (without the
  <NUL> character) \return validation status: 0 if specified name is invalid, >0
  otherwise
*/
...
/**
  ...
  Microsoft specifies MAX_PATH as: drive letter, colon, backslash, name
  components separated by backslashes, and a terminating null character.

  The parameter path can start with the backslash following by name components
  and therefore we determine the max path len by excluding drive letter and a
  colon.

  \param[in]  path                      character string containing path
  information
...
static uint32_t read_cache(fsFAT_Volume *vol, uint32_t sect, uint32_t cnt) {
    ...
    if (vol->ca.nrd > 0) {
        if ((vol->ca.csect <= sect) && sect < (vol->ca.csect + vol->ca.nrd)) {
            ...
            memcpy(vol->ca.buf, vol->ca.cbuf + (sect - vol->ca.csect) * 512,
                   512);
            vol->ca.sect = sect;
            return (true);
        }
...
static uint32_t write_cache(fsFAT_Volume *vol, uint32_t sect) {
    ...
    if (sect == 0) {
        /* Flush cache request. */
        if (vol->ca.nwr) {
            if (vol->Drv->WriteSect(vol->ca.csect, vol->ca.cbuf, vol->ca.nwr) ==
                false) {
                /* Sector write failed */
                EvrFsFAT_SectorWriteFailed(vol->DrvLet, vol->ca.csect,
                                           vol->ca.nwr);
                return (false);
            }
            vol->ca.nwr = 0;
    ...
    vol->ca.sect = sect;
    if (vol->ca.nwr > 0) {
        if (sect == (vol->ca.csect + vol->ca.nwr) &&
            vol->ca.nwr < vol->CaSize) {
            /* Next sector is continuous, still space in cache. */
            memcpy(vol->ca.cbuf + (vol->ca.nwr * 512), vol->ca.buf, 512);
            vol->ca.nwr++;
            return (true);
        } else if (sect >= (vol->ca.csect) &&
                   sect < (vol->ca.csect + vol->ca.nwr)) {
            /* Sector is already in the cache */
            memcpy(vol->ca.cbuf + ((sect - vol->ca.csect) * 512), vol->ca.buf,
                   512);
            return (true);
        }
...
static uint32_t cache_fat(fsFAT_Volume *vol, uint32_t sect) {
    ...
    if (vol->fat.dirty == true) {
        ...
        if (vol->fsj && vol->Status & FAT_STATUS_JOURACT) {
            ...
            ofs = (vol->cfg.NumOfFat == 2) ? (vol->fat.sect + vol->cfg.FatSize)
                                           : (0);
            if (fsj_write(vol->fsj, vol->fat.sect, ofs, vol->fat.buf) ==
                false) {
                return (false);
            }
        } else {
            ...
            if (vol->cfg.NumOfFat == 2) {
                /* Write copy of FAT */
                if (vol->Drv->WriteSect(vol->fat.sect + vol->cfg.FatSize,
                                        vol->fat.buf, 1) == false) {
                    /* Sector write failed */
                    EvrFsFAT_SectorWriteFailed(
                        vol->DrvLet, vol->fat.sect + vol->cfg.FatSize, 1);
                    return (false);
                }
            }
        }
...
/**
  ...
  \param[in]  vol                       volume description structure
*/
static uint32_t alloc_table_read(uint32_t clus, uint32_t *val,
                                 fsFAT_Volume *vol) {
...
*/
static uint32_t alloc_table_write(uint32_t clus, uint32_t val,
                                  fsFAT_Volume *vol) {
    ...
    switch (vol->cfg.FatType) {
        case FS_FAT12:
            ...
            if (offs < 511) {
                ...
                if (clus & 0x001) {
                    vol->fat.buf[511] =
                        (uint8_t)((vol->fat.buf[511] & 0x0F) | (link << 4));
                } else {
                ...
                if (clus & 0x001) {
                    ...
                    vol->fat.buf[0] =
                        (uint8_t)((vol->fat.buf[0] & 0xF0) | (link >> 8));
                }
            }
            break;
...
*/
static uint32_t link_clus(uint32_t clus, uint32_t next_clus,
                          fsFAT_Volume *vol) {
...
*/
static uint32_t follow_clus(uint32_t *clus, uint32_t *link, uint32_t cnt,
                            fsFAT_Volume *vol) {
    uint32_t eoc, n, val;
...
static uint32_t entry_type(FILEREC *de) {
    if (de->FileName[0] == 0x00) {
        ...
        if ((de->Attr & (FS_FAT_ATTR_DIRECTORY | FS_FAT_ATTR_VOLUME_ID)) ==
...
/**
  ...
  \param[in]  start_clus                first cluster of the directory we are
  searching in \param[in]  cnt                       number of entries to
  allocate \param[out] rpos                      record position \param[in]  vol
  volume description structure

  \return     - 0: Ok, entries allocated
              - 1: Failed, cannot allocate (no space, FAT12/16 root limit,
  driver error)
*/
static uint32_t frec_allocate(uint32_t start_clus, uint32_t cnt, ENTRY_POS *pos,
                              fsFAT_Volume *vol) {
    ELINK *el;
    FILEREC *frec;
    ENTRY_POS cur_pos;
...
static fsStatus frec_delete_elink(FREC_POS *rpos, fsFAT_Volume *vol) {
    ...
    if (el) {
        if (entry_pos_comp(&rpos->pos, el)) {
            ...
            EvrFsFAT_NameCacheEntryDelete(vol->DrvLet, el->Info.EntryClus,
                                          el->Info.EntryOffs,
                                          el->Info.EntryCount);
        }
    }
...
*/
static fsStatus frec_next(PATH_INFO *pinfo, char *fn, uint32_t len,
                          fsFAT_Volume *vol) {
#ifndef FS_FAT_NO_LFN
    uint8_t chksum;
    uint8_t valid;
...
/* Process SFN entry */
                ...
                if (ord) {
                    ...
                    if (ents == valid) {
                        if (chksum ==
                            sn_chksum((char *)pinfo->sfne->FileName)) {
                ...
                if (el) {
                    ...
                    el->Info.ShortNameH =
                        short_ent_hash(pinfo->sfne, el->Info.ShortNameH);
                    ...
                    EvrFsFAT_NameCacheEntryInsert(
                        vol->DrvLet, el->Info.EntryClus, el->Info.EntryOffs,
                        el->Info.EntryCount);
                }
...
static fsStatus frec_find_elink(PATH_INFO *pinfo, fsFAT_Volume *vol) {
    ...
    if (vol->ncache) {
        ...
        if (elink_scan(hash, &el, vol->ncache) == 0) {
            EvrFsFAT_NameCacheEntryFound(vol->DrvLet, el->Info.EntryClus,
                                         el->Info.EntryOffs,
                                         el->Info.EntryCount);
            ...
            switch (entry_type(sfne)) {
                ...
                case ENTRY_TYPE_LFN:
                    ...
                    if (lfne->Ordinal & ORD_LONG_NAME_LAST) {
                        ...
                        if (pinfo->fn_len !=
                            ((ents - 1) * 13 + lfn_char_cnt(lfne))) {
                            /* Name length different, clear checksum and
                             * continue processing */
                            chksum = 0U;
                        }
                    }
...
/* Process SFN entry */
                    ...
                    if (el) {
                        el->Info.LongNameH = hash;
                        el->Info.ShortNameH =
                            short_ent_hash(sfne, el->Info.ShortNameH);
                        el->Info.DirClus = pinfo->dir_clus;
                        el->Info.EntryCount = pinfo->frec.cnt;
...
/**
  ...
  \return \ref fsStatus
*/
static fsStatus create_lfn_entry(PATH_INFO *pinfo, uint8_t sfn_chksum,
                                 fsFAT_Volume *vol) {
    LFN_FILEREC *frec;
    bool ord;
    uint8_t seq;
...
/**
  ...
  \return \ref fsStatus
*/
static fsStatus create_sfn_entry(const char *fn, ENTRY_POS *pos,
                                 uint32_t first_clus, uint8_t attr,
                                 fsFAT_Volume *vol) {
    FILEREC *frec;
    fsTime time;
    uint32_t i;
...
*/
static fsStatus create_dot_entries(uint32_t dir_clus, uint32_t first_clus,
                                   fsFAT_Volume *vol) {
    FILEREC *frec;
    ENTRY_POS pos;
    fsTime time;
...
static uint32_t mbr_read(fsFAT_Volume *vol) {
    ...
    if ((vol->ca.buf[0] == 0xE9) ||
        (vol->ca.buf[0] == 0xEB && vol->ca.buf[2] == 0x90)) {
...
*/
static uint32_t label_write(fsFAT_Volume *vol, const char *label, uint32_t len,
                            uint32_t sect) {
    FILEREC *frec;
    fsTime time;
    uint32_t i;
...
*/
static uint32_t j_mark_rsvd(fsFAT_Volume *vol, uint32_t first_clus,
                            uint32_t cnt) {
...
*/
static uint32_t j_find_reg(fsFAT_Volume *vol, uint32_t *first_clus,
                           uint32_t *num_of) {
...
*/
static uint32_t j_chk_rsvd(fsFAT_Volume *vol, uint32_t first_clus,
                           uint32_t cnt) {
...
static uint32_t j_rep_space(fsFAT_Volume *vol, uint32_t first_clus) {
    ...
    uint32_t cnt =
        vol->cfg.SecPerClus * j_clus_cnt(vol->RsvdS, vol->cfg.SecPerClus) - 1;
    ...
    for (clus = vol->cfg.DataClusCnt + 1; clus > low; clus--) {
        ...
        if (fen_val == get_RSVD(vol->cfg.FatType)) {
            ...
            if (j_is_signed(vol, clus) == true) {
                ...
                if (j_chk_rsvd(vol, first,
                               j_clus_cnt(vol->RsvdS, vol->cfg.SecPerClus)) ==
                    true) {
                    *first_clus = first;
...
static fsStatus path_open(PATH_INFO *pinfo, fsFAT_Volume *vol) {
    ...
    do {
        ...
        if (!(pinfo->fn_flags & FAT_NAME_DOT)) {
            ...
            if ((pinfo->fn_flags & (FAT_NAME_DIR | FAT_NAME_LAST)) ==
                FAT_NAME_DIR) {
                ...
                if (pinfo->fn_flags & FAT_NAME_DOTDOT) {
                    ...
                    elink_dir(pinfo->dir_clus, ELINK_DIR_DEPTH_DEC,
                              vol->ncache);
                } else {
                    /* Increment name cache path depth */
                    elink_dir(pinfo->dir_clus, ELINK_DIR_DEPTH_INC,
                              vol->ncache);
                }
            }
        }
...
static fsStatus path_create(PATH_INFO *pinfo, bool mkdir, fsFAT_Volume *vol) {
    ...
    do {
        ...
        err = frec_allocate(pinfo->dir_clus, pinfo->frec.cnt, &pinfo->frec.pos,
                            vol);
        if (err) {
            /* Cannot allocate */
            return (fsError);
        ...
        /* Create short entry */
        if (create_sfn_entry(sn, &pinfo->frec.pos, first_clus, attrib, vol) !=
            fsOK) {
            return (fsError);
        }
        ...
        if (el) {
            el->Info.ShortNameH =
                short_ent_hash((FILEREC *)(uint32_t)sn, el->Info.ShortNameH);
            el->Info.DirClus = pinfo->dir_clus;
            el->Info.EntryCount = pinfo->frec.cnt;
            ...
            EvrFsFAT_NameCacheEntryInsert(vol->DrvLet, el->Info.EntryClus,
                                          el->Info.EntryOffs,
                                          el->Info.EntryCount);
        }
...
__WEAK fsStatus fat_mount(fsFAT_Volume *vol) {
    ...
    root_scnt = (vol->cfg.RootEntCnt * 32 + vol->cfg.BytesPerSec - 1) /
                vol->cfg.BytesPerSec;

    /* Count Data Sectors/Clusters */
    vol->cfg.DataSecCnt =
        vol->cfg.DskSize - (vol->cfg.RsvdSecCnt +
                            vol->cfg.NumOfFat * vol->cfg.FatSize + root_scnt);
    vol->cfg.DataClusCnt = vol->cfg.DataSecCnt / vol->cfg.SecPerClus;
    vol->cfg.RootDirAddr =
        vol->cfg.RsvdSecCnt + vol->cfg.NumOfFat * vol->cfg.FatSize;
...
/**
  ...
  \param[in]  vol                       volume description structure
  \param[in]  opt                       formatting options as a NULL terminated
  string \return     execution status \ref fsStatus
*/
__WEAK fsStatus fat_format(fsFAT_Volume *vol, const char *opt) {
    ...
    /* Check volume status */
    stat = fat_vol_chk(
        FAT_STATUS_INIT_MEDIA | FAT_STATUS_READY | FAT_STATUS_WRITE, vol);
    if (stat != fsOK) {
        return (stat);
    }
    ...
    if ((ptr = find_param(opt, "/LLEB")) != NULL) {
         ...
         */
        if (vol->Drv->DeviceCtrl(fsDevCtrlCodeFormat, (char *)(uint32_t)ptr) !=
            fsOK) {
            return (fsDriverError);
        }
    } else {
        ...
        if (find_param(opt, "/LL") != NULL) {
            ...
            /* Low level format the device. Parameter is passed to a device
             * driver */
    ...
    switch (vol->cfg.FatType) {
        case FS_FAT12:
            ...
            vol->cfg.FatSize =
                (uint32_t)((vol->cfg.DataClusCnt * 3 + 1022) / 1024);
            break;
        case FS_FAT16:
            datSect -= 32;
            vol->cfg.RootEntCnt = 512;
    ...
    EvrFsFAT_FormatProperties(vol->DrvLet, vol->cfg.FatType, vol->cfg.ClusSize,
                              vol->cfg.DataClusCnt);

    /* Count Data Sectors/Clusters */
    vol->cfg.DataSecCnt =
        vol->cfg.DskSize - (vol->cfg.RsvdSecCnt + vol->cfg.RootSecCnt +
    ...
    if (mbr) {
        ...
        vol->cfg.BootSector =
            ((vol->cfg.BootSector + sec) & (uint32_t)~0x3F) - sec;
    ...
    for (i = 1; i < vol->cfg.RsvdSecCnt; i++) {
        ...
        if (vol->cfg.FatType == FS_FAT32) {
            if ((i == vol->cfg.FAT32_FSInfo) ||
                (i == vol->cfg.FAT32_BkBootSec)) {
...
__WEAK fsStatus fat_open(int32_t handle, const char *path, int32_t openmode) {
    ...
    status = fat_vol_chk(FAT_STATUS_INIT_IO | FAT_STATUS_INIT_MEDIA |
                             FAT_STATUS_READY | FAT_STATUS_MOUNT,
                         fh->vol);
...
__WEAK fsStatus fat_close(int32_t handle) {
    ...
    if (fh->flags & (FAT_HANDLE_DATA_RD | FAT_HANDLE_DATA_WR)) {
        /* Check volume status */
        stat = fat_vol_chk(
            FAT_STATUS_READY | FAT_STATUS_MOUNT | FAT_STATUS_WRITE, fh->vol);
        if (stat != fsOK) {
        ...
        if (fh->flags & FAT_HANDLE_DATA_WR) {
            ...
            set_u16((uint8_t *)&frec->FirstClusHI,
                    (uint16_t)(fh->first_clus >> 16));
...
__WEAK int32_t fat_read(int32_t handle, uint8_t *buf, uint32_t len) {
    ...
    status = fat_vol_chk(
        FAT_STATUS_REMOVABLE | FAT_STATUS_READY | FAT_STATUS_MOUNT, fh->vol);
    if (status != fsOK) {
        fh->flags |= FAT_HANDLE_ERROR;
    ...
    for (nr = 0; nr < len; nr += rlen) {
        if (fh->current_sect == fh->vol->cfg.SecPerClus) {
            ...
            if (alloc_table_read(fh->current_clus, &fh->current_clus,
                                 fh->vol) == false) {
                /* Read error */
        ...
        if (read_cache(fh->vol, sect,
                       fh->vol->cfg.SecPerClus - fh->current_sect) == false) {
            /* Read error */
            return (-(int32_t)fsDriverError);
        }
...
__WEAK int64_t fat_seek(int32_t handle, int64_t offset, int32_t whence) {
    ...
    if (pos > fh->fcsz) {
        ...
        if (fh->first_clus == 0) {
            ...
            if (follow_clus(&fh->current_clus, &link,
                            get_EOC(fh->vol->cfg.FatType), fh->vol) == false) {
                /* R/W error */
                return -(int64_t)(fsDriverError);
            }
            clus = fh->current_clus;
...
/**
  ...
  \return     execution status \ref fsStatus
*/
__WEAK fsStatus fat_delete(const char *path, const char *options,
                           fsFAT_Volume *vol) {
    PATH_INFO pinfo;
    fsStatus status;
    uint32_t clus, edir;
    ...
    status = fat_vol_chk(FAT_STATUS_READY | FAT_STATUS_MOUNT | FAT_STATUS_WRITE,
                         vol);
    if (status != fsOK) {
        return (status);
    }
...
/**
  ...
  \return     execution status \ref fsStatus
*/
__WEAK fsStatus fat_rename(const char *path, const char *newname,
                           fsFAT_Volume *vol) {
    PATH_INFO pinfo;
    FREC_POS ent;
    FILEREC old_entry;
    ...
    if (status == fsOK) {
        ...
        if (status == fsFileNotFound) {
            ...
            if (status == fsOK) {
                ...
                fs_get_time(&time);
                set_u16((uint8_t *)&pinfo.sfne->LastAccDate,
                        format_date(&time));
            ...
            if ((pinfo.frec.pos.Offs != ent.pos.Offs) ||
                (pinfo.frec.pos.Clus != ent.pos.Clus)) {
...
/**
  ...
  \return     execution status \ref fsStatus
*/
__WEAK fsStatus fat_rmdir(const char *path, const char *options,
                          fsFAT_Volume *vol) {
    PATH_INFO pinfo;
    FREC_POS fpos;
    fsStatus status;
...
__WEAK fsStatus fat_info(fsDriveInfo *info, fsFAT_Volume *vol) {
    ...
    if (info == NULL) {
        ...
        if (status == fsOK) {
            switch (vol->cfg.FatType) {
                case FS_RAW:
                    info->fs_type = fsTypeNone;
                    break;
                case FS_FAT12:
                    info->fs_type = fsTypeFAT12;
                    break;
                case FS_FAT16:
                    info->fs_type = fsTypeFAT16;
                    break;
                case FS_FAT32:
                    info->fs_type = fsTypeFAT32;
                    break;
...
/**
  ...
  maximum path length.
  If \a len argument is less than maximum path length, function returns with
  \ref fsInvalidParameter and content of \a path remains unchanged.
...
/**
  ...
  \return     execution status \ref fsStatus
*/
__WEAK fsStatus fat_attrib(fsFAT_Volume *vol, const char *path,
                           uint32_t attrib) {
    PATH_INFO pinfo;
    fsStatus status;
...
/**
  ...
  \param[in]  path                      string specifying the file or directory
  path. \param[in]  create                    structure specifying the file
  creation time \param[in]  access                    structure specifying the
  file last access time \param[in]  write                     structure
  specifying the file last write time \param[in]  vol volume description
  structure \return     execution status \ref fsStatus
*/
__WEAK fsStatus fat_time_set(const char *path, fsTime *create, fsTime *access,
                             fsTime *write, fsFAT_Volume *vol) {
    PATH_INFO pinfo;
    fsStatus status;
...
/**
  ...
  \param[in]  path                      string specifying the file or directory
  path. \param[out] create                    structure where file creation time
  will be stored \param[out] access                    structure where file last
  access time will be stored \param[out] write                     structure
  where file last write time will be stored \param[in]  vol volume description
  structure \return     execution status \ref fsStatus
*/
__WEAK fsStatus fat_time_get(const char *path, fsTime *create, fsTime *access,
                             fsTime *write, fsFAT_Volume *vol) {
    PATH_INFO pinfo;
    fsStatus status;
