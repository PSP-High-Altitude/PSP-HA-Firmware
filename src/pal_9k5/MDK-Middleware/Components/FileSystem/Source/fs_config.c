...
#define FS_NDRV                                                           \
    (NOR0_ENABLE + NOR1_ENABLE + RAM0_ENABLE + RAM1_ENABLE + MC0_ENABLE + \
     MC1_ENABLE + USB0_ENABLE + USB1_ENABLE + NAND0_ENABLE + NAND1_ENABLE)
#if (FS_NDRV == 0)
#error \
    "No drive enabled, File System requires at least one component ::File System:Drive"
#endif

/* Count number of enabled FAT drives */
#define FS_FAT_NDRV                                                        \
    (MC0_ENABLE + MC1_ENABLE + NAND0_ENABLE + NAND1_ENABLE + RAM0_ENABLE + \
     RAM1_ENABLE + USB0_ENABLE + USB1_ENABLE)
...
#error \
    "::File System:Drive:Memory Card: MCI ports for different drives must be different"
#endif
#elif ((MC0_SPI == 1) && (MC1_SPI == 1))
#if (MC0_SPI_DRIVER == MC1_SPI_DRIVER)
#error \
    "::File System:Drive:Memory Card: SPI ports for different drives must be different"
#endif
#endif
#endif
...
#endif
#ifndef RAM0_FAT_JOURNAL
#define RAM0_FAT_JOURNAL 0
#endif
#ifndef RAM1_FAT_JOURNAL
#define RAM1_FAT_JOURNAL 0
#endif

#define FSJ_USE                                                            \
    (MC0_ENABLE * MC0_FAT_JOURNAL + MC1_ENABLE * MC1_FAT_JOURNAL +         \
     NAND0_ENABLE * NAND0_FAT_JOURNAL + NAND1_ENABLE * NAND1_FAT_JOURNAL + \
     USB0_ENABLE * USB0_FAT_JOURNAL + USB1_ENABLE * USB1_FAT_JOURNAL)
...
#define NCACHE_USE                                                       \
    (MC0_NAME_CACHE_SIZE + MC1_NAME_CACHE_SIZE + NAND0_NAME_CACHE_SIZE + \
     NAND1_NAME_CACHE_SIZE + USB0_NAME_CACHE_SIZE + USB1_NAME_CACHE_SIZE)
...
#define __IN_SECTION_DEF(name, num) \
    __attribute__((section(name __PP_STRINGIFY(num))))
#endif
...
static FLASH_TIMEOUT fs_nor0_flash_tout = {NOR0_TOUT_ERASE_CHIP,
                                           NOR0_TOUT_ERASE_SECTOR,
                                           NOR0_TOUT_PROGRAM, NOR0_TOUT_READ};
...
static fsStatus nor0_Init(void) { return (nor_Init(&fs_nor0)); }
static fsStatus nor0_UnInit(void) { return (nor_Uninit(&fs_nor0)); }
...
static EFS_DRV fs_nor0_drv = {
    nor0_Init,      nor0_UnInit,   nor0_GetSectorAddress, nor0_GetSectorSize,
    nor0_DataWrite, nor0_DataRead, nor0_SectorErase,      nor0_DevCtrl};
...
static FLASH_TIMEOUT fs_nor1_flash_tout = {NOR1_TOUT_ERASE_CHIP,
                                           NOR1_TOUT_ERASE_SECTOR,
                                           NOR1_TOUT_PROGRAM, NOR1_TOUT_READ};
...
static fsStatus nor1_Init(void) { return (nor_Init(&fs_nor1)); }
static fsStatus nor1_UnInit(void) { return (nor_Uninit(&fs_nor1)); }
...
static EFS_DRV fs_nor1_drv = {
    nor1_Init,      nor1_UnInit,   nor1_GetSectorAddress, nor1_GetSectorSize,
    nor1_DataWrite, nor1_DataRead, nor1_SectorErase,      nor1_DevCtrl};
#endif
...
static uint32_t mc0_cache[(MC0_CACHE_SIZE + 1) * 256 +
                          MC0_FAT_JOURNAL * 128] __ALIGNED(32) __SECTION_MC0;

#if (MC0_NAME_CACHE_SIZE > 0)
#define MC0_NAME_CACHE_MAX_DEPTH 8
#define MC0_NAME_CACHE_BUF_SIZE                   \
    ((MC0_NAME_CACHE_SIZE * FAT_NCACHE_LINK_SZ) + \
     (MC0_NAME_CACHE_MAX_DEPTH * FAT_NCACHE_USED_SZ) + FAT_NCACHE_STAT_SZ)

static uint32_t mc0_ncache_buf[MC0_NAME_CACHE_BUF_SIZE / 4];

static NCACHE mc0_ncache = {MC0_NAME_CACHE_MAX_DEPTH, MC0_NAME_CACHE_BUF_SIZE,
                            mc0_ncache_buf};
#endif
...
static uint32_t mc0_Init(uint32_t mode) { return (MC0_INIT(mode)); }
static uint32_t mc0_UnInit(uint32_t mode) { return (MC0_UNINIT(mode)); }
...
static uint32_t mc0_RdInfo(fsMediaInfo *info) { return (MC0_RDINFO(info)); }
static fsStatus mc0_DevCtrl(fsDevCtrlCode code, void *p) {
    return (MC0_DEVCTRL(code, p));
}

/* MC0 FAT device driver */
static FAT_DRV fs_mc0_drv = {mc0_Init,   mc0_UnInit, mc0_RdSect,
                             mc0_WrSect, mc0_RdInfo, mc0_DevCtrl};
...
static uint32_t mc1_cache[(MC1_CACHE_SIZE + 1) * 256 +
                          MC1_FAT_JOURNAL * 128] __ALIGNED(32) __SECTION_MC1;

#if (MC1_NAME_CACHE_SIZE > 0)
#define MC1_NAME_CACHE_MAX_DEPTH 8
#define MC1_NAME_CACHE_BUF_SIZE                   \
    ((MC1_NAME_CACHE_SIZE * FAT_NCACHE_LINK_SZ) + \
     (MC1_NAME_CACHE_MAX_DEPTH * FAT_NCACHE_USED_SZ) + FAT_NCACHE_STAT_SZ)

static uint32_t mc1_ncache_buf[MC1_NAME_CACHE_BUF_SIZE / 4];

static NCACHE mc1_ncache = {MC1_NAME_CACHE_MAX_DEPTH, MC1_NAME_CACHE_BUF_SIZE,
                            mc1_ncache_buf};
#endif
...
static uint32_t mc1_Init(uint32_t mode) { return (MC1_INIT(mode)); }
static uint32_t mc1_UnInit(uint32_t mode) { return (MC1_UNINIT(mode)); }
...
static uint32_t mc1_RdInfo(fsMediaInfo *info) { return (MC1_RDINFO(info)); }
static fsStatus mc1_DevCtrl(fsDevCtrlCode code, void *p) {
    return (MC1_DEVCTRL(code, p));
}

/* MC1 FAT device driver */
static FAT_DRV fs_mc1_drv = {mc1_Init,   mc1_UnInit, mc1_RdSect,
                             mc1_WrSect, mc1_RdInfo, mc1_DevCtrl};
...
#error \
    "::File System:Drive:NAND: NAND ports for different drives must be different"
#endif
...
static void NAND0_HW_SignalEventHandler(uint32_t dev_num, uint32_t event) {
    ...
    switch (dev_num) {
        case NAND0_DEV_NUM:
            nand_DrvEvent(event, &fs_nand0_media_handle);
            break;

#if defined(NAND0_DEVICE1)
        case NAND1_DEV_NUM:
            nand_DrvEvent(event, &fs_nand1_media_handle);
            break;
#endif
...
static void NAND0_Media_SignalEventHandler(uint32_t dev_num, uint32_t event) {
    ...
    switch (dev_num) {
        case NAND0_DEV_NUM:
            ftl_DrvEvent(event, &fs_nand0_handle);
            break;

#if defined(NAND0_DEVICE1)
        case NAND1_DEV_NUM:
            ftl_DrvEvent(event, &fs_nand1_handle);
            break;
#endif
...
#if (((NAND0_ENABLE == 0) && NAND1_ENABLE) || \
     ((NAND0_ENABLE && NAND1_ENABLE) && (NAND1_DRIVER != NAND0_DRIVER)))
...
#define NAND0_NAME_CACHE_BUF_SIZE                   \
    ((NAND0_NAME_CACHE_SIZE * FAT_NCACHE_LINK_SZ) + \
     (NAND0_NAME_CACHE_MAX_DEPTH * FAT_NCACHE_USED_SZ) + FAT_NCACHE_STAT_SZ)

static uint32_t nand0_ncache_buf[NAND0_NAME_CACHE_BUF_SIZE / 4];

static NCACHE nand0_ncache = {NAND0_NAME_CACHE_MAX_DEPTH,
                              NAND0_NAME_CACHE_BUF_SIZE, nand0_ncache_buf};
#endif
...
static NAND_FTL_CFG nand0_cfg = {
    ...
    &nand0_cabl[0],
    (uint8_t *)&nand0_cache[(NAND0_CACHE_SIZE + 1) * 256 +
                            (NAND0_PAGE_CACHE + 2) * NAND0_PAGE_SIZE / 4],
    &nand0_capg[0],
...
static FAT_DRV fs_nand0_drv = {nand0_Init,   nand0_UnInit, nand0_RdSect,
                               nand0_WrSect, nand0_RdInfo, nand0_DevCtrl};
#endif
...
#define NAND1_NAME_CACHE_BUF_SIZE                   \
    ((NAND1_NAME_CACHE_SIZE * FAT_NCACHE_LINK_SZ) + \
     (NAND1_NAME_CACHE_MAX_DEPTH * FAT_NCACHE_USED_SZ) + FAT_NCACHE_STAT_SZ)

static uint32_t nand1_ncache_buf[NAND1_NAME_CACHE_BUF_SIZE / 4];

static NCACHE nand1_ncache = {NAND1_NAME_CACHE_MAX_DEPTH,
                              NAND1_NAME_CACHE_BUF_SIZE, nand1_ncache_buf};
#endif
...
static NAND_FTL_CFG nand1_cfg = {
    ...
    &nand1_cabl[0],
    (uint8_t *)&nand1_cache[(NAND1_CACHE_SIZE + 1) * 256 +
                            (NAND1_PAGE_CACHE + 2) * NAND1_PAGE_SIZE / 4],
    &nand1_capg[0],
...
static FAT_DRV fs_nand1_drv = {nand1_Init,   nand1_UnInit, nand1_RdSect,
                               nand1_WrSect, nand1_RdInfo, nand1_DevCtrl};
#endif
...
#endif
fsFAT_Volume fs_ram0_vol = {0};
#if (RAM0_FAT_JOURNAL)
static FSJOUR fs_ram0_fsj;
#endif
...
/* NAND Cache Buffer for FAT, Page and Block Caching */
#define RAM0_CSZ ((4 + 1) * 1024 + (1 + 2) * 4096 + (1 + 2) * 64)
#define RAM0_FSJBUF (RAM0_FAT_JOURNAL * 512)

static uint32_t ram0_cache[RAM0_CSZ / 4 + RAM0_FSJBUF / 4] __ALIGNED(32)
    __SECTION_RAM0;
...
static
#endif
    RAM_DEV fs_ram0_dev = {NULL, RAM0_SIZE};
// RAM_DEV fs_ram0_dev = {(uint8_t *)&ram0_buf[256], RAM0_SIZE};
...
static FAT_DRV fs_ram0_drv = {ram0_Init,   ram0_UnInit, ram0_RdSect,
                              ram0_WrSect, ram0_RdInfo, ram0_DevCtrl};
#endif
...
static
#endif
    RAM_DEV fs_ram1_dev = {(uint8_t *)&ram1_buf[256], RAM1_SIZE};
...
/* FAT RAM Driver Control Block */
static FAT_DRV fs_ram1_drv = {ram1_Init,   ram1_UnInit, ram1_RdSect,
                              ram1_WrSect, ram1_RdInfo, ram1_DevCtrl};
#endif
...
#define USB0_NAME_CACHE_BUF_SIZE                   \
    ((USB0_NAME_CACHE_SIZE * FAT_NCACHE_LINK_SZ) + \
     (USB0_NAME_CACHE_MAX_DEPTH * FAT_NCACHE_USED_SZ) + FAT_NCACHE_STAT_SZ)

static uint32_t usb0_ncache_buf[USB0_NAME_CACHE_BUF_SIZE / 4];

static NCACHE usb0_ncache = {USB0_NAME_CACHE_MAX_DEPTH,
                             USB0_NAME_CACHE_BUF_SIZE, usb0_ncache_buf};
#endif

/* USB Cache Buffer for Data and FAT Caching */
static uint32_t
    usb0_cache[(USB0_CACHE_SIZE + 1) * 256 + USB0_FAT_JOURNAL * 128];

/* USB0 wrapper functions */
static uint32_t usb0_Init(uint32_t mode) { return (usbh_msc_Init(mode, 0)); }
...
static FAT_DRV fs_usb0_drv = {usb0_Init,   usb0_UnInit, usb0_RdSect,
                              usb0_WrSect, usb0_RdInfo, usb0_DevCtrl};
#endif
...
/* USB Cache Buffer for Data and FAT Caching */
static uint32_t
    usb1_cache[(USB1_CACHE_SIZE + 1) * 256 + USB1_FAT_JOURNAL * 128];

#if (USB1_NAME_CACHE_SIZE > 0)
#define USB1_NAME_CACHE_MAX_DEPTH 8
#define USB1_NAME_CACHE_BUF_SIZE                   \
    ((USB1_NAME_CACHE_SIZE * FAT_NCACHE_LINK_SZ) + \
     (USB1_NAME_CACHE_MAX_DEPTH * FAT_NCACHE_USED_SZ) + FAT_NCACHE_STAT_SZ)

static uint32_t usb1_ncache_buf[USB1_NAME_CACHE_BUF_SIZE / 4];

static NCACHE usb1_ncache = {USB1_NAME_CACHE_MAX_DEPTH,
                             USB1_NAME_CACHE_BUF_SIZE, usb1_ncache_buf};
#endif

/* USB1 wrapper functions */
static uint32_t usb1_Init(uint32_t mode) { return (usbh_msc_Init(mode, 1)); }
...
static FAT_DRV fs_usb1_drv = {usb1_Init,   usb1_UnInit, usb1_RdSect,
                              usb1_WrSect, usb1_RdInfo, usb1_DevCtrl};
#endif
...
#if (RAM0_FAT_JOURNAL)
            /* Register file system journal */
            fs_ram0_fsj.buf = (uint8_t *)&ram0_cache[RAM0_CSZ / 4];
            fs_ram0_vol.fsj = &fs_ram0_fsj;
            fs_ram0_vol.RsvdS = FAT_SECT_RSVD;
#else
        fs_ram0_vol.fsj = NULL;
        fs_ram0_vol.RsvdS = 0;
#endif
...
__STATIC_INLINE void fs_config_usb(char num) {
    switch (num) {
        ...
        case '0':
            ...
            /* Register file system journal */
            fs_usb0_fsj.buf =
                (uint8_t *)&usb0_cache[(USB0_CACHE_SIZE + 1) * 256];
        ...
        case '1':
            ...
            /* Register file system journal */
            fs_usb1_fsj.buf =
                (uint8_t *)&usb1_cache[(USB1_CACHE_SIZE + 1) * 256];
 ...
 *----------------------------------------------------------------------------*/
#if (EFS_USE == 0)
int32_t efs_handle_get(fsEFS_Volume *v) {
    (void)v;
    return (-1);
}
fsStatus efs_init(fsEFS_Volume *v) {
    (void)v;
    return (fsError);
}
fsStatus efs_uninit(fsEFS_Volume *v) {
    (void)v;
    return (fsError);
}
fsStatus efs_mount(fsEFS_Volume *v) {
    (void)v;
    return (fsError);
}
fsStatus efs_unmount(fsEFS_Volume *v) {
    (void)v;
    return (fsError);
}
fsStatus efs_open(int32_t h, const char *p, int32_t a) {
    (void)h;
    (void)p;
    (void)a;
    return (fsError);
}
fsStatus efs_close(int32_t h) {
    (void)h;
    return (fsError);
}
int32_t efs_read(int32_t h, uint8_t *p, uint32_t a) {
    (void)h;
    (void)p;
    (void)a;
    return (0);
}
int32_t efs_write(int32_t h, const uint8_t *p, uint32_t a) {
    (void)h;
    (void)p;
    (void)a;
    return (-1);
}
fsStatus efs_flush(int32_t h) {
    (void)h;
    return (fsError);
}
int32_t efs_flen(int32_t h) {
    (void)h;
    return (-1);
}
int32_t efs_seek(int32_t h, int32_t o, int32_t w) {
    (void)h;
    (void)o;
    (void)w;
    return (fsError);
}
fsStatus efs_ffind(fsFileInfo *i, fsEFS_Volume *v) {
    (void)i;
    (void)v;
    return (fsError);
}
fsStatus efs_rename(const char *p, const char *n, fsEFS_Volume *v) {
    (void)p;
    (void)n;
    (void)v;
    return (fsError);
}
fsStatus efs_delete(const char *p, fsEFS_Volume *v) {
    (void)p;
    (void)v;
    return (fsError);
}
int32_t efs_analyse(fsEFS_Volume *v) {
    (void)v;
    return (0);
}
fsStatus efs_check(fsEFS_Volume *v) {
    (void)v;
    return (fsError);
}
fsStatus efs_format(fsEFS_Volume *v) {
    (void)v;
    return (fsError);
}
int64_t efs_free(fsEFS_Volume *v) {
    (void)v;
    return (-1);
}
fsStatus efs_defrag(fsEFS_Volume *v) {
    (void)v;
    return (fsError);
}
fsStatus efs_info(fsDriveInfo *i, fsEFS_Volume *v) {
    (void)i;
    (void)v;
    return (fsError);
}
...
#if (FAT_USE == 0)
int32_t fat_handle_get(fsFAT_Volume *v) {
    (void)v;
    return (-1);
}
fsStatus fat_init(fsFAT_Volume *v) {
    (void)v;
    return (fsError);
}
fsStatus fat_uninit(fsFAT_Volume *v) {
    (void)v;
    return (fsError);
}
fsStatus fat_mount(fsFAT_Volume *v) {
    (void)v;
    return (fsError);
}
fsStatus fat_unmount(fsFAT_Volume *v) {
    (void)v;
    return (fsError);
}
fsStatus fat_open(int32_t h, const char *p, int32_t a) {
    (void)h;
    (void)p;
    (void)a;
    return (fsError);
}
fsStatus fat_close(int32_t h) {
    (void)h;
    return (fsError);
}
int32_t fat_read(int32_t h, uint8_t *p, uint32_t a) {
    (void)h;
    (void)p;
    (void)a;
    return (-1);
}
int32_t fat_write(int32_t h, const uint8_t *p, uint32_t a) {
    (void)h;
    (void)p;
    (void)a;
    return (-1);
}
fsStatus fat_flush(int32_t h) {
    (void)h;
    return (fsError);
}
int32_t fat_flen(int32_t h) {
    (void)h;
    return (-1);
}
int64_t fat_seek(int32_t h, int64_t o, int32_t w) {
    (void)h;
    (void)o;
    (void)w;
    return (fsError);
}
fsStatus fat_ffind(const char *p, fsFileInfo *i, fsFAT_Volume *v) {
    (void)p;
    (void)i;
    (void)v;
    return (fsError);
}
fsStatus fat_delete(const char *p, const char *o, fsFAT_Volume *v) {
    (void)p;
    (void)o;
    (void)v;
    return (fsError);
}
fsStatus fat_rename(const char *p, const char *n, fsFAT_Volume *v) {
    (void)p;
    (void)n;
    (void)v;
    return (fsError);
}
fsStatus fat_format(fsFAT_Volume *v, const char *o) {
    (void)v;
    (void)o;
    return (fsError);
}
int64_t fat_free(fsFAT_Volume *v) {
    (void)v;
    return (-1);
}
fsStatus fat_attrib(fsFAT_Volume *v, const char *p, uint32_t a) {
    (void)v;
    (void)p;
    (void)a;
    return (fsError);
}
fsStatus fat_vol(char *l, uint32_t *s, fsFAT_Volume *v) {
    (void)l;
    (void)s;
    (void)v;
    return (fsError);
}
fsStatus fat_pwd(char *p, uint32_t l, fsFAT_Volume *v) {
    (void)p;
    (void)l;
    (void)v;
    return (fsError);
}
fsStatus fat_media(fsFAT_Volume *v) {
    (void)v;
    return (fsError);
}
fsStatus fat_info(fsDriveInfo *i, fsFAT_Volume *v) {
    (void)i;
    (void)v;
    return (fsError);
}
fsStatus fat_chdir(const char *p, fsFAT_Volume *v) {
    (void)p;
    (void)v;
    return (fsError);
}
fsStatus fat_mkdir(const char *p, fsFAT_Volume *v) {
    (void)p;
    (void)v;
    return (fsError);
}
fsStatus fat_rmdir(const char *p, const char *o, fsFAT_Volume *v) {
    (void)p;
    (void)o;
    (void)v;
    return (fsError);
}
fsStatus fat_time_set(const char *p, fsTime *c, fsTime *a, fsTime *w,
                      fsFAT_Volume *v) {
    (void)p;
    (void)c;
    (void)a;
...
fsStatus fat_time_get(const char *p, fsTime *c, fsTime *a, fsTime *w,
                      fsFAT_Volume *v) {
    (void)p;
    (void)c;
    (void)a;
...
uint32_t fat_jour_init(fsFAT_Volume *v) {
    (void)v;
    return (true);
}
#else
...
uint32_t fat_jour_prep(fsFAT_Volume *v) {
    (void)v;
    return (true);
}
uint32_t fsj_set_space(FSJOUR *p, uint32_t s, uint32_t c) {
    (void)p;
    (void)s;
    (void)c;
    return (false);
}
uint32_t fsj_write(FSJOUR *p, uint32_t s, uint32_t c, uint8_t *b) {
    (void)p;
    (void)s;
    (void)c;
    (void)b;
...
uint32_t fsj_init(FSJOUR *p, FAT_DRV *d) {
    (void)p;
    (void)d;
    return (false);
}
...
uint32_t elink_init(void *p) {
    (void)p;
    return (1);
}
uint32_t elink_dir(uint32_t i, uint32_t j, void *p) {
    (void)i;
    (void)j;
    (void)p;
    return (1);
}
uint32_t elink_cmd(uint32_t i, void *p) {
    (void)i;
    (void)p;
    return (0);
}
uint32_t elink_insert(void *e, void *p) {
    (void)e;
    (void)p;
    return (1);
}
uint32_t elink_delete(void *e, void *p) {
    (void)e;
    (void)p;
    return (0);
}
uint32_t elink_scan(uint32_t i, void **e, void *p) {
    (void)i;
    (void)e;
    (void)p;
    return (1);
