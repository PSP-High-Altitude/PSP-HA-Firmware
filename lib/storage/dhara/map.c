...
int dhara_map_read(struct dhara_map *m, dhara_sector_t s, uint8_t *data,
                   dhara_error_t *err) {
...
static int raw_gc(struct dhara_map *m, dhara_page_t src, dhara_error_t *err) {
    ...
    if (trace_path(m, target, &current, meta, &my_err) < 0) {
        if (my_err == DHARA_E_NOT_FOUND) return 0;
...
static int prepare_write(struct dhara_map *m, dhara_sector_t dst, uint8_t *meta,
                         dhara_error_t *err) {
    dhara_error_t my_err;
...
int dhara_map_copy_sector(struct dhara_map *m, dhara_sector_t src,
    ...
    if (dhara_map_find(m, src, &p, &my_err) < 0) {
        if (my_err == DHARA_E_NOT_FOUND) return dhara_map_trim(m, dst, err);
...
int dhara_map_sync(struct dhara_map *m, dhara_error_t *err) {
    while (!dhara_journal_is_clean(&m->journal)) {
        ...
        if (p == DHARA_PAGE_NONE) {
            ...
            ret = raw_gc(m, p, &my_err);
            if (!ret) dhara_journal_dequeue(&m->journal);
        }
