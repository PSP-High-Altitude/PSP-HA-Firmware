...
static int yaffs_wr_nand(struct yaffs_dev *dev, int nand_chunk, const u8 *data,
                         struct yaffs_spare *spare) {
    int data_size = dev->data_bytes_per_chunk;

    return dev->drv.drv_write_chunk_fn(dev, nand_chunk, data, data_size,
                                       (u8 *)spare, sizeof(*spare));
}

static int yaffs_rd_chunk_nand(struct yaffs_dev *dev, int nand_chunk, u8 *data,
                               struct yaffs_spare *spare,
    ...
    if (dev->param.use_nand_ecc)
        return dev->drv.drv_read_chunk_fn(dev, nand_chunk, data, data_size,
                                          (u8 *)spare, spare_size, ecc_result);

    /* Handle the ECC at this level. */

    ret_val = dev->drv.drv_read_chunk_fn(dev, nand_chunk, data, data_size,
                                         (u8 *)spare, spare_size, NULL);
    if (!data || !correct_errors) return ret_val;
    ...
    if (ecc_result1 > 0) {
        ...
        yaffs_trace(YAFFS_TRACE_ERROR,
                    "**>>yaffs ecc error unfixed on chunk %d:0", nand_chunk);
        dev->n_ecc_unfixed++;
    }
    ...
    if (ecc_result2 > 0) {
        ...
        yaffs_trace(YAFFS_TRACE_ERROR,
                    "**>>yaffs ecc error unfixed on chunk %d:1", nand_chunk);
        dev->n_ecc_unfixed++;
    }
...
static void yaffs_handle_rd_data_error(struct yaffs_dev *dev, int nand_chunk) {
    ...
    yaffs_get_block_info(dev, flash_block + dev->block_offset)->needs_retiring =
        1;
    yaffs_trace(YAFFS_TRACE_ERROR | YAFFS_TRACE_BAD_BLOCKS,
                "**>>Block %d marked for retirement", flash_block);
...
static int yaffs_tags_compat_rd(struct yaffs_dev *dev, int nand_chunk, u8 *data,
...
static int yaffs_tags_compat_mark_bad(struct yaffs_dev *dev, int flash_block) {
    ...
    yaffs_wr_nand(dev, flash_block * dev->param.chunks_per_block + 1, NULL,
                  &spare);

    return YAFFS_OK;
}

static int yaffs_tags_compat_query_block(struct yaffs_dev *dev, int block_no,
