...
static int yaffs_tags_marshall_write(struct yaffs_dev *dev, int nand_chunk,
    ...
    retval = dev->drv.drv_write_chunk_fn(
        dev, nand_chunk, data, dev->param.total_bytes_per_chunk,
...
static int yaffs_tags_marshall_read(struct yaffs_dev *dev, int nand_chunk,
    ...
    yaffs_trace(YAFFS_TRACE_MTD,
                "yaffs_tags_marshall_read chunk %d data %p tags %p", nand_chunk,
                data, tags);
    ...
    if (dev->param.inband_tags || (data && !tags))
        retval = dev->drv.drv_read_chunk_fn(dev, nand_chunk, data,
                                            dev->param.total_bytes_per_chunk,
                                            NULL, 0, &ecc_result);
    else if (tags)
        retval = dev->drv.drv_read_chunk_fn(
            dev, nand_chunk, data, dev->param.total_bytes_per_chunk,
            spare_buffer, packed_tags_size, &ecc_result);
    else
        BUG();
...
static int yaffs_tags_marshall_query_block(struct yaffs_dev *dev, int block_no,
    ...
    if (retval == YAFFS_FAIL) {
        ...
        *seq_number = 0;
        ...
        yaffs_tags_marshall_read(dev, block_no * dev->param.chunks_per_block,
                                 NULL, &t);
    ...
    yaffs_trace(YAFFS_TRACE_MTD, "block query returns  seq %d state %d",
                *seq_number, *state);
