...
int yaffs_obj_cache_dirty(struct yaffs_obj *obj) {
    ...
    for (i = 0; i < mgr->n_caches; i++) {
        ...
        if (cache->object == obj && cache->dirty) return 1;
    }
...
void yaffs_flush_single_cache(struct yaffs_cache *cache, int discard) {
    if (!cache || cache->locked) return;

    /* Write it out and free it up  if need be.*/
    if (cache->dirty) {
        yaffs_wr_data_obj(cache->object, cache->chunk_id, cache->data,
                          cache->n_bytes, 1);

        cache->dirty = 0;
    }

    if (discard) cache->object = NULL;
}
...
void yaffs_flush_file_cache(struct yaffs_obj *obj, int discard) {
    ...
    if (mgr->n_caches < 1) return;

    /* Find the chunks for this object and flush them. */
    for (i = 0; i < mgr->n_caches; i++) {
        struct yaffs_cache *cache = &mgr->cache[i];

        if (cache->object == obj) yaffs_flush_single_cache(cache, discard);
    }
}
...
void yaffs_flush_whole_cache(struct yaffs_dev *dev, int discard) {
    ...
    do {
        ...
        for (i = 0; i < mgr->n_caches && !obj; i++) {
            struct yaffs_cache *cache = &mgr->cache[i];
            if (cache->object && cache->dirty) obj = cache->object;
        }
        if (obj) yaffs_flush_file_cache(obj, discard);
    } while (obj);
}
...
struct yaffs_cache *yaffs_grab_chunk_cache(struct yaffs_dev *dev) {
    ...
    for (i = 0; i < mgr->n_caches; i++) {
        ...
        if (this_cache->object && !this_cache->locked &&
...
struct yaffs_cache *yaffs_find_chunk_cache(const struct yaffs_obj *obj,
    ...
    for (i = 0; i < mgr->n_caches; i++) {
        ...
        if (cache->object == obj && cache->chunk_id == chunk_id) {
            dev->cache_hits++;
            return cache;
        }
    }
...
void yaffs_use_cache(struct yaffs_dev *dev, struct yaffs_cache *cache,
    ...
    if (mgr->cache_last_use < 0 || mgr->cache_last_use > 100000000) {
        ...
        for (i = 1; i < mgr->n_caches; i++) mgr->cache[i].last_use = 0;
...
void yaffs_invalidate_file_cache(struct yaffs_obj *in) {
    ...
    for (i = 0; i < mgr->n_caches; i++) {
        ...
        if (cache->object == in) cache->object = NULL;
    }
}
...
int yaffs_cache_init(struct yaffs_dev *dev) {
    ...
    if (mgr->n_caches > 0) {
        ...
        if (mgr->cache) memset(mgr->cache, 0, cache_bytes);
...
void yaffs_cache_deinit(struct yaffs_dev *dev) {
    ...
    if (mgr->n_caches < 1 || !mgr->cache) return;
