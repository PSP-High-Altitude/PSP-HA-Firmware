...
void yaffs_addr_to_chunk(struct yaffs_dev *dev, loff_t addr, int *chunk_out,
                         u32 *offset_out) {
    int chunk;
    u32 offset;
...
u8 *yaffs_get_temp_buffer(struct yaffs_dev *dev) {
    ...
    dev->temp_in_use++;
    if (dev->temp_in_use > dev->max_temp) dev->max_temp = dev->temp_in_use;
...
struct yaffs_obj *yaffs_root(struct yaffs_dev *dev) { return dev->root_dir; }
...
static int yaffs_verify_chunk_written(struct yaffs_dev *dev, int nand_chunk,
    ...
    // if (nand_chunk / dev->param.chunks_per_block == 1) {
    //     printf("result = %d\n", result);
    //     for (int i = 0; i < dev->data_bytes_per_chunk; i++) {
    //         if (i % 16 == 0) printf("\n");
    //         if (buffer[i] != data[i]) printf("%d != %d ", buffer[i],
    //         data[i]);
    //     }
    // }
...
static int yaffs_find_alloc_block(struct yaffs_dev *dev) {
    ...
    for (i = dev->internal_start_block; i <= dev->internal_end_block; i++) {
        ...
        if (bi->block_state == YAFFS_BLOCK_STATE_EMPTY) {
            ...
            dev->n_erased_blocks--;
            yaffs_trace(
                YAFFS_TRACE_ALLOCATE, "Allocated block %d, seq  %d, %d left",
                dev->alloc_block_finder, dev->seq_number, dev->n_erased_blocks);
...
static int yaffs_alloc_chunk(struct yaffs_dev *dev, int use_reserver,
    ...
    if (dev->alloc_block >= 0) {
        ...
        ret_val =
            (dev->alloc_block * dev->param.chunks_per_block) + dev->alloc_page;
        bi->pages_in_use++;
...
static int yaffs_write_new_chunk(struct yaffs_dev *dev, const u8 *data,
                                 struct yaffs_ext_tags *tags,
                                 int use_reserver) {
    u32 attempts = 0;
    int write_ok = 0;
    ...
    do {
        ...
        if (dev->param.always_check_erased) bi->skip_erased_check = 0;
    ...
    if (attempts > 1) {
        yaffs_trace(YAFFS_TRACE_ERROR, "**>> yaffs write required %d attempts",
                    attempts);
...
static void yaffs_retire_block(struct yaffs_dev *dev, int flash_block) {
    ...
    if (yaffs_mark_bad(dev, flash_block) != YAFFS_OK) {
        if (yaffs_erase_block(dev, flash_block) != YAFFS_OK) {
            ...
            if (dev->tagger.write_chunk_tags_fn(dev,
                                                chunk_id - dev->chunk_offset,
                                                buffer, &tags) != YAFFS_OK)
                yaffs_trace(
                    YAFFS_TRACE_ALWAYS,
...
void yaffs_set_obj_name(struct yaffs_obj *obj, const YCHAR *name) {
    ...
    if (name && !name[0]) {
        ...
        name = obj->short_name;
    } else if (name && strnlen(name, YAFFS_SHORT_NAME_LENGTH + 1) <=
    ...
    if (l > 0) {
        while (l > 0 && tn) {
            ...
            if ((l > 1) && !tn->internal[x]) {
                ...
                if (!tn->internal[x]) return NULL;
            } else if (l == 1) {
                /* Looking from level 1 at level 0 */
                if (passed_tn) {
                    /* If we already have one, release it */
                    if (tn->internal[x]) yaffs_free_tnode(dev, tn->internal[x]);
...
static int yaffs_tags_match(const struct yaffs_ext_tags *tags, int obj_id,
                            int chunk_obj) {
    return (tags->chunk_id == (u32)chunk_obj && tags->obj_id == (u32)obj_id &&
            !tags->is_deleted)
               ? 1
               : 0;
}
...
static int yaffs_find_del_file_chunk(struct yaffs_obj *in, int inode_chunk,
    ...
    if (ret_val != -1) yaffs_load_tnode_0(dev, tn, inode_chunk, 0);
...
int yaffs_put_chunk_in_file(struct yaffs_obj *in, int inode_chunk,
    ...
    tn = yaffs_add_find_tnode_0(dev, &in->variant.file_variant, inode_chunk,
                                NULL);
    if (!tn) return YAFFS_FAIL;

    if (!nand_chunk) /* Dummy insert, bail now */
        return YAFFS_OK;
    ...
    if (in_scan != 0) {
        ...
        if (existing_cunk > 0) {
            ...
            if (in_scan > 0) {
                ...
                /* Do a proper find */
                existing_cunk =
                    yaffs_find_chunk_in_file(in, inode_chunk, &existing_tags);
            }
    ...
    if (existing_cunk == 0) in->n_data_chunks++;
...
static void yaffs_remove_obj_from_dir(struct yaffs_obj *obj) {
    ...
    if (dev && dev->param.remove_obj_fn) dev->param.remove_obj_fn(obj);
...
static int yaffs_change_obj_name(struct yaffs_obj *obj,
                                 ...
                                 const YCHAR *new_name, int force,
                                 int shadows) {
    int unlink_op;
    int del_op;
    struct yaffs_obj *existing_target;

    if (new_dir == NULL) new_dir = obj->parent; /* use the old directory */
...
static void yaffs_free_obj(struct yaffs_obj *obj) {
    ...
    dev = obj->my_dev;
    yaffs_trace(YAFFS_TRACE_OS, "FreeObject %p inode %p", obj, obj->my_inode);
    if (obj->parent) BUG();
    if (!list_empty(&obj->siblings)) BUG();
...
void yaffs_handle_defered_free(struct yaffs_obj *obj) {
    if (obj->defered_free) yaffs_free_obj(obj);
}
...
static void yaffs_soft_del_file(struct yaffs_obj *obj) {
    if (!obj->deleted || obj->variant_type != YAFFS_OBJECT_TYPE_FILE ||
        obj->soft_del)
        return;
    ...
    if (obj->n_data_chunks <= 0) {
        ...
        yaffs_trace(YAFFS_TRACE_TRACING, "yaffs: Deleting empty file %d",
                    obj->obj_id);
        yaffs_generic_obj_del(obj);
    } else {
        yaffs_soft_del_worker(obj, obj->variant.file_variant.top,
                              obj->variant.file_variant.top_level, 0);
        obj->soft_del = 1;
    }
}
...
static struct yaffs_tnode *yaffs_prune_worker(struct yaffs_dev *dev,
    ...
    if (level > 0) {
        for (i = 0; i < YAFFS_NTNODES_INTERNAL; i++) {
            if (tn->internal[i]) {
                tn->internal[i] = yaffs_prune_worker(
                    dev, tn->internal[i], level - 1, (i == 0) ? del0 : 1);
            }
...
static int yaffs_prune_tree(struct yaffs_dev *dev,
    ...
    if (file_struct->top_level < 1) return YAFFS_OK;
...
static struct yaffs_obj *yaffs_alloc_empty_obj(struct yaffs_dev *dev) {
     ...
     */
    if (dev->lost_n_found) yaffs_add_obj_to_dir(dev->lost_n_found, obj);
...
struct yaffs_obj *yaffs_find_by_number(struct yaffs_dev *dev, u32 number) {
    ...
    list_for_each(i, &dev->obj_bucket[bucket].list) {
        ...
        if (in->obj_id == number) {
            ...
            if (in->defered_free) return NULL;
            return in;
        }
    }
...
static struct yaffs_obj *yaffs_new_obj(struct yaffs_dev *dev, int number,
    ...
    if (number < 0) number = yaffs_new_obj_id(dev);
...
struct yaffs_obj *yaffs_find_or_create_by_number(struct yaffs_dev *dev,
    ...
    if (number > 0) the_obj = yaffs_find_by_number(dev, number);

    if (!the_obj) the_obj = yaffs_new_obj(dev, number, type);
    ...
    if (dev->param.defered_dir_update) {
        ...
        if (list_empty(link)) {
            ...
            yaffs_trace(YAFFS_TRACE_BACKGROUND,
                        "Added object %d to dirty directories", obj->obj_id);
        }
...
void yaffs_update_dirty_dirs(struct yaffs_dev *dev) {
    ...
    while (!list_empty(&dev->dirty_dirs)) {
        ...
        yaffs_trace(YAFFS_TRACE_BACKGROUND, "Update directory %d", obj->obj_id);

        if (obj->dirty) yaffs_update_oh(obj, NULL, 0, 0, 0, NULL);
    }
}
...
static struct yaffs_obj *yaffs_create_obj(enum yaffs_obj_type type,
                                          ...
                                          u32 gid, struct yaffs_obj *equiv_obj,
    ...
    if (yaffs_find_by_name(parent, name)) return NULL;
    ...
    if (in) yaffs_update_parent(parent);
...
struct yaffs_obj *yaffs_create_file(struct yaffs_obj *parent, const YCHAR *name,
    ...
    return yaffs_create_obj(YAFFS_OBJECT_TYPE_FILE, parent, name, mode, uid,
                            gid, NULL, NULL, 0);
}

struct yaffs_obj *yaffs_create_dir(struct yaffs_obj *parent, const YCHAR *name,
                                   u32 mode, u32 uid, u32 gid) {
    return yaffs_create_obj(YAFFS_OBJECT_TYPE_DIRECTORY, parent, name, mode,
                            uid, gid, NULL, NULL, 0);
}
...
struct yaffs_obj *yaffs_create_special(struct yaffs_obj *parent,
                                       const YCHAR *name, u32 mode, u32 uid,
                                       u32 gid, u32 rdev) {
    return yaffs_create_obj(YAFFS_OBJECT_TYPE_SPECIAL, parent, name, mode, uid,
                            gid, NULL, NULL, rdev);
}
...
struct yaffs_obj *yaffs_create_symlink(struct yaffs_obj *parent,
    ...
    return yaffs_create_obj(YAFFS_OBJECT_TYPE_SYMLINK, parent, name, mode, uid,
                            gid, NULL, alias, 0);
}
...
struct yaffs_obj *yaffs_link_obj(struct yaffs_obj *parent, const YCHAR *name,
    ...
    if (yaffs_create_obj(YAFFS_OBJECT_TYPE_HARDLINK, parent, name, 0, 0, 0,
                         equiv_obj, NULL, 0))
        return equiv_obj;
...
static int yaffs_init_blocks(struct yaffs_dev *dev) {
    ...
    if (!dev->block_info) goto alloc_error;
    ...
    if (!dev->chunk_bits) goto alloc_error;
...
void yaffs_block_became_dirty(struct yaffs_dev *dev, int block_no) {
    ...
    yaffs_trace(YAFFS_TRACE_GC | YAFFS_TRACE_ERASE,
                "yaffs_block_became_dirty block %d state %d %s", block_no,
                bi->block_state, (bi->needs_retiring) ? "needs retiring" : "");
    ...
    if (block_no == (int)dev->gc_block) dev->gc_block = 0;
    ...
    if (erased_ok && ((yaffs_trace_mask & YAFFS_TRACE_ERASE) ||
...
static inline int yaffs_gc_process_chunk(struct yaffs_dev *dev,
    ...
    yaffs_trace(YAFFS_TRACE_GC_DETAIL,
                "Collecting chunk in block %d, %d %d %d ", dev->gc_chunk,
    ...
    if (object && !yaffs_skip_verification(dev)) {
        if (tags.chunk_id == 0)
            ...
            matching_chunk =
                yaffs_find_chunk_in_file(object, tags.chunk_id, NULL);

        if (old_chunk != matching_chunk)
            yaffs_trace(YAFFS_TRACE_ERROR,
                        "gc: page in gc mismatch: %d %d %d %d", old_chunk,
                        matching_chunk, tags.obj_id, tags.chunk_id);
    }

    if (!object) {
        yaffs_trace(YAFFS_TRACE_ERROR, "page %d in gc has no object: %d %d %d ",
...
static int yaffs_gc_block(struct yaffs_dev *dev, int block, int whole_block) {
    ...
    if (is_checkpt_block || !yaffs_still_some_chunks(dev, block)) {
        yaffs_trace(YAFFS_TRACE_TRACING,
                    "Collecting block %d that has no chunks in use", block);
...
static unsigned yaffs_find_gc_block(struct yaffs_dev *dev, int aggressive,
                                    int background) {
    u32 i;
    u32 iterations;
    u32 selected = 0;
    ...
    if (!selected) {
        ...
        for (i = 0;
             i < iterations && (dev->gc_dirtiest < 1 ||
    ...
    if (selected) {
        yaffs_trace(
            YAFFS_TRACE_GC, "GC Selected block %d with %d free, prioritised:%d",
            selected, dev->param.chunks_per_block - dev->gc_pages_in_use,
            prioritised);

        dev->n_gc_blocks++;
        if (background) dev->bg_gcs++;

        dev->gc_dirtiest = 0;
        dev->gc_pages_in_use = 0;
        dev->gc_not_done = 0;
        if (dev->refresh_skip > 0) dev->refresh_skip--;
    } else {
        dev->gc_not_done++;
        yaffs_trace(YAFFS_TRACE_GC,
                    "GC none: finder %d skip %d threshold %d dirtiest %d using "
                    "%d oldest %d%s",
...
static int yaffs_check_gc(struct yaffs_dev *dev, int background) {
    ...
    if (dev->gc_disable) /* Bail out so we don't get recursive gc */
        return YAFFS_OK;
    ...
    do {
        ...
        if (dev->n_erased_blocks < min_erased)
            ...
            if (erased_chunks < dev->n_free_chunks / 2 || dev->gc_skip < 1 ||
                background)
                aggressive = 0;
            else {
                dev->gc_skip--;
        ...
        if (dev->n_erased_blocks < (int)dev->param.n_reserved_blocks &&
            dev->gc_block > 0) {
            yaffs_trace(YAFFS_TRACE_GC,
                        "yaffs: GC !!!no reclaim!!! n_erased_blocks %d after "
                        "try %d block %d",
                        dev->n_erased_blocks, max_tries, dev->gc_block);
        }
...
static int yaffs_rd_data_obj(struct yaffs_obj *in, int inode_chunk,
    ...
    if (nand_chunk >= 0)
        ...
        yaffs_trace(YAFFS_TRACE_NANDACCESS, "Chunk %d not found zero instead",
                    nand_chunk);
...
void yaffs_chunk_del(struct yaffs_dev *dev, int chunk_id, int mark_flash,
    ...
    if (!yaffs_check_chunk_bit(dev, block, page))
        yaffs_trace(YAFFS_TRACE_VERIFY, "Deleting invalid chunk %d", chunk_id);

    bi = yaffs_get_block_info(dev, block);

    yaffs2_update_oldest_dirty_seq(dev, block, bi);

    yaffs_trace(YAFFS_TRACE_DELETION, "line %d delete of chunk %d", lyn,
                chunk_id);
...
int yaffs_wr_data_obj(struct yaffs_obj *in, int inode_chunk, const u8 *buffer,
                      int n_bytes, int use_reserve) {
    ...
    if (n_bytes < 1 || n_bytes > (int)dev->data_bytes_per_chunk) {
        yaffs_trace(YAFFS_TRACE_ERROR, "Writing %d bytes to chunk!!!!!!!!!",
                    n_bytes);
        BUG();
    }
...
static int yaffs_apply_xattrib_mod(struct yaffs_obj *obj, char *buffer,
    ...
    if (xmod->set)
        retval = nval_set(dev, x_buffer, x_size, xmod->name, xmod->data,
...
static int yaffs_do_xattrib_fetch(struct yaffs_obj *obj, const YCHAR *name,
    ...
    if (obj->hdr_chunk < 1) return -ENODATA;
    ...
    if (result != YAFFS_OK)
        ...
        if (name)
            retval = nval_get(dev, x_buffer, x_size, name, value, size);
        else
...
static void yaffs_check_obj_details_loaded(struct yaffs_obj *in) {
    ...
    if (!in || !in->lazy_loaded || in->hdr_chunk < 1) return;
...
int yaffs_update_oh(struct yaffs_obj *in, const YCHAR *name, int force,
    ...
    if (prev_chunk_id > 0) {
        ...
        result =
            yaffs_rd_chunk_tags_nand(dev, prev_chunk_id, buffer, &old_tags);
        if (result == YAFFS_OK) {
    ...
    if (xmod) yaffs_apply_xattrib_mod(in, (char *)buffer, xmod);
    ...
    new_chunk_id = yaffs_write_new_chunk(dev, buffer, &new_tags,
                                         (prev_chunk_id > 0) ? 1 : 0);
    ...
    if (!yaffs_obj_cache_dirty(in)) in->dirty = 0;

    /* If this was a shrink, then mark the block
     * that the chunk lives on */
    if (is_shrink) {
        bi = yaffs_get_block_info(
            in->my_dev, new_chunk_id / in->my_dev->param.chunks_per_block);
        bi->has_shrink_hdr = 1;
    }
...
int yaffs_do_file_wr(struct yaffs_obj *in, const u8 *buffer, loff_t offset,
    ...
    while (n > 0 && chunk_written >= 0) {
        ...
        if ((start + n) < dev->data_bytes_per_chunk) {
            ...
            n_writeback =
                (n_bytes_read > (start + n)) ? n_bytes_read : (start + n);

            if (n_writeback < 0 || n_writeback > (int)dev->data_bytes_per_chunk)
                BUG();
        ...
        if (n_copy != (int)dev->data_bytes_per_chunk ||
            !dev->param.cache_bypass_aligned || dev->param.inband_tags) {
            ...
            if (dev->param.n_caches > 0) {
                ...
                chunk_written =
                    yaffs_wr_data_obj(in, chunk, local_buffer, n_writeback, 0);
            ...
            chunk_written = yaffs_wr_data_obj(in, chunk, buffer,
...
static void yaffs_prune_chunks(struct yaffs_obj *in, loff_t new_size) {
    ...
    for (i = last_del; i >= start_del; i--) {
        ...
        if ((u32)chunk_id <
            ...
            (u32)chunk_id >=
                ((dev->internal_end_block + 1) * dev->param.chunks_per_block)) {
            yaffs_trace(YAFFS_TRACE_ALWAYS, "Found daft chunk_id %d for %d",
                        chunk_id, i);
        } else {
            in->n_data_chunks--;
...
int yaffs_resize_file(struct yaffs_obj *in, loff_t new_size) {
    ...
    if (in->variant_type != YAFFS_OBJECT_TYPE_FILE) return YAFFS_FAIL;

    if (new_size == old_size) return YAFFS_OK;
...
int yaffs_flush_file(struct yaffs_obj *in, int update_time, int data_sync,
                     int discard_cache) {
    if (!in->dirty) return YAFFS_OK;

    yaffs_flush_file_cache(in, discard_cache);

    if (data_sync) return YAFFS_OK;

    if (update_time) yaffs_load_current_time(in, 0, 0);

    return (yaffs_update_oh(in, NULL, 0, 0, 0, NULL) >= 0) ? YAFFS_OK
                                                           : YAFFS_FAIL;
}
...
static int yaffs_unlink_file_if_needed(struct yaffs_obj *in) {
    ...
    if (del_now) {
        ret_val =
            yaffs_change_obj_name(in, in->my_dev->del_dir, _Y("deleted"), 0, 0);
        yaffs_trace(YAFFS_TRACE_TRACING, "yaffs: immediate deletion of file %d",
                    in->obj_id);
        in->deleted = 1;
        ...
        ret_val = yaffs_change_obj_name(in, in->my_dev->unlinked_dir,
                                        _Y("unlinked"), 0, 0);
    }
    return ret_val;
}
...
static int yaffs_del_file(struct yaffs_obj *in) {
    ...
    if (in->n_data_chunks > 0) {
         ...
         */
        if (!in->unlinked) ret_val = yaffs_unlink_file_if_needed(in);
...
static int yaffs_del_dir(struct yaffs_obj *obj) {
    ...
    if (yaffs_is_non_empty_dir(obj)) return YAFFS_FAIL;
...
static int yaffs_unlink_worker(struct yaffs_obj *obj) {
    ...
    if (obj->variant_type == YAFFS_OBJECT_TYPE_HARDLINK) {
        ...
        if (ret_val == YAFFS_OK) ret_val = yaffs_generic_obj_del(hl);
...
int yaffs_unlink_obj(struct yaffs_obj *obj) {
    if (obj && obj->unlink_allowed) return yaffs_unlink_worker(obj);
...
int yaffs_rename_obj(struct yaffs_obj *old_dir, const YCHAR *old_name,
    ...
    if (strnlen(new_name, YAFFS_MAX_NAME_LENGTH + 1) > YAFFS_MAX_NAME_LENGTH)
        /* ENAMETOOLONG */
        return YAFFS_FAIL;
    ...
    if (obj && obj->rename_allowed) {
        ...
        if (new_dir != old_dir) yaffs_update_parent(new_dir);
...
static int yaffs_has_null_parent(struct yaffs_dev *dev, struct yaffs_obj *obj) {
    return (obj == dev->del_dir || obj == dev->unlinked_dir ||
            obj == dev->root_dir);
}
    ...
    for (i = 0; i < YAFFS_NOBJECT_BUCKETS; i++) {
        list_for_each_safe(lh, n, &dev->obj_bucket[i].list) {
            ...
            if (yaffs_has_null_parent(dev, obj)) {
                ...
                if (parent != dev->root_dir) hanging = 1;
            }
            if (hanging) {
...
static void yaffs_del_dir_contents(struct yaffs_obj *dir) {
    ...
    if (dir->variant_type != YAFFS_OBJECT_TYPE_DIRECTORY) BUG();

    list_for_each_safe(lh, n, &dir->variant.dir_variant.children) {
        obj = list_entry(lh, struct yaffs_obj, siblings);
        if (obj->variant_type == YAFFS_OBJECT_TYPE_DIRECTORY)
            yaffs_del_dir_contents(obj);
        yaffs_trace(YAFFS_TRACE_SCAN, "Deleting lost_found object %d",
                    obj->obj_id);
...
struct yaffs_obj *yaffs_find_by_name(struct yaffs_obj *directory,
    ...
    list_for_each(i, &directory->variant.dir_variant.children) {
        ...
        if (l->parent != directory) BUG();

        yaffs_check_obj_details_loaded(l);

        /* Special case for lost-n-found */
        if (l->obj_id == YAFFS_OBJECTID_LOSTNFOUND) {
            if (!strcmp(name, YAFFS_LOSTNFOUND_NAME)) return l;
        } else if (l->sum == sum || l->hdr_chunk <= 0) {
            /* LostnFound chunk called Objxxx
             * Do a real check
             */
            yaffs_get_obj_name(l, buffer, YAFFS_MAX_NAME_LENGTH + 1);
            if (!strncmp(name, buffer, YAFFS_MAX_NAME_LENGTH)) return l;
        }
    }
    return NULL;
}
...
int yaffs_get_obj_name(struct yaffs_obj *obj, YCHAR *name, int buffer_size) {
    ...
    if (obj->obj_id == YAFFS_OBJECTID_LOSTNFOUND) {
        ...
        if (obj->hdr_chunk > 0) {
            result = yaffs_rd_chunk_tags_nand(obj->my_dev, obj->hdr_chunk,
                                              buffer, NULL);
            if (result == YAFFS_OK)
...
loff_t yaffs_get_obj_length(struct yaffs_obj *obj) {
    ...
    if (obj->variant_type == YAFFS_OBJECT_TYPE_SYMLINK) {
        if (!obj->variant.symlink_variant.alias) return 0;
...
int yaffs_get_obj_link_count(struct yaffs_obj *obj) {
    ...
    if (!obj->unlinked) count++; /* the object itself */
...
static int yaffs_check_dev_fns(struct yaffs_dev *dev) {
    ...
    if (!drv->drv_read_chunk_fn || !drv->drv_write_chunk_fn ||
        !drv->drv_erase_fn)
        return 0;
    ...
    if (!tagger->write_chunk_tags_fn || !tagger->read_chunk_tags_fn ||
        !tagger->query_block_fn || !tagger->mark_bad_fn)
        return 0;
...
static int yaffs_create_initial_dir(struct yaffs_dev *dev) {
    ...
    dev->lost_n_found = yaffs_create_fake_dir(dev, YAFFS_OBJECTID_LOSTNFOUND,
...
int yaffs_guts_ll_init(struct yaffs_dev *dev) {
    ...
    if (dev->ll_init) return YAFFS_OK;
    ...
    if ((!dev->param.inband_tags && dev->param.is_yaffs2 &&
        ...
        dev->param.chunks_per_block < 2 || dev->param.n_reserved_blocks < 2 ||
        dev->internal_start_block <= 0 || dev->internal_end_block <= 0 ||
        dev->internal_end_block <=
            (dev->internal_start_block + dev->param.n_reserved_blocks + 2)) {
        /* otherwise it is too small */
        yaffs_trace(YAFFS_TRACE_ALWAYS,
                    "NAND geometry problems: chunk size %d, type is yaffs%s, "
                    "inband_tags %d ",
    ...
    if (!yaffs_init_tmp_buffers(dev)) return YAFFS_FAIL;
...
int yaffs_guts_format_dev(struct yaffs_dev *dev) {
    ...
    if (dev->is_mounted) return YAFFS_FAIL;

    for (i = dev->internal_start_block; i <= dev->internal_end_block; i++) {
        yaffs_query_init_block_state(dev, i, &state, &dummy);
        if (state != YAFFS_BLOCK_STATE_DEAD) yaffs_erase_block(dev, i);
    }
...
void yaffs_guts_cleanup(struct yaffs_dev *dev) {
    ...
    if (dev->param.empty_lost_n_found) yaffs_empty_l_n_f(dev);
}
...
int yaffs_guts_initialise(struct yaffs_dev *dev) {
    ...
    if (!init_failed) init_failed = yaffs_cache_init(dev) < 0;

    dev->cache_hits = 0;

    if (!init_failed) {
        dev->gc_cleanup_list =
            kmalloc(dev->param.chunks_per_block * sizeof(u32), GFP_NOFS);
        if (!dev->gc_cleanup_list) init_failed = 1;
    }

    if (dev->param.is_yaffs2) dev->param.use_header_file_size = 1;
    ...
    if (!init_failed && dev->param.is_yaffs2 && !dev->param.disable_summary &&
    ...
    for (b = dev->internal_start_block; b <= dev->internal_end_block; b++) {
        switch (blk->block_state) {
            ...
            case YAFFS_BLOCK_STATE_FULL:
                n_free += (dev->param.chunks_per_block - blk->pages_in_use +
                           blk->soft_del_pages);
                break;
            default:
                break;
...
loff_t yaffs_oh_to_size(struct yaffs_dev *dev, struct yaffs_obj_hdr *oh,
    ...
    if (sizeof(loff_t) >= 8 && ~(oh->file_size_high)) {
        ...
        if (do_endian) yaffs_do_endian_u32(dev, &low);
        retval = (loff_t)low;
    }
